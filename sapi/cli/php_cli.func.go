		...
		*arg_excp = arg_free
		...
		case PHP_MODE_PROCESS_STDIN:
			...
			for exit_status == zend.SUCCESS && b.Assign(&input, core.PhpStreamGets(SInProcess, nil, 0)) != nil {
				...
				zend.__EG().GetSymbolTable().KeyUpdate(b.CastStr("argn", b.SizeOf("\"argn\"")-1), &argn)
				zend.ZVAL_LONG(&argi, b.PreInc(&index))
				zend.__EG().GetSymbolTable().KeyUpdate(b.CastStr("argi", b.SizeOf("\"argi\"")-1), &argi)
				if exec_run != nil {
