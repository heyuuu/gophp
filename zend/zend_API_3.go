	...
	if !is_method || this_ptr == nil || !this_ptr.IsObject() {
		...
		*object = this_ptr
		if ce != nil && operators.InstanceofFunction(types.Z_OBJCE_P(this_ptr), ce) == 0 {
			faults.ErrorNoreturn(faults.E_CORE_ERROR, fmt.Sprintf("%s::%s() must be derived from %s::%s", types.Z_OBJCE_P(this_ptr).Name(), CurrEX().FunctionName(), ce.Name(), CurrEX().FunctionName()))
		}
