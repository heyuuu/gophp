	...
	if !is_method || this_ptr == nil || this_ptr.GetType() != types.IS_OBJECT {
		...
		*object = this_ptr
		if ce != nil && operators.InstanceofFunction(types.Z_OBJCE_P(this_ptr), ce) == 0 {
			faults.ErrorNoreturn(faults.E_CORE_ERROR, "%s::%s() must be derived from %s::%s", types.Z_OBJCE_P(this_ptr).GetName().GetVal(), GetActiveFunctionName(), ce.Name(), GetActiveFunctionName())
		}
...
func ZendUpdateClassConstants(class_type *types.ClassEntry) int {
	if !class_type.IsConstantsUpdated() {
		...
		if class_type.GetDefaultStaticMembersCount() != 0 && CE_STATIC_MEMBERS(class_type) == nil {
			if class_type.IsInternalClass() || class_type.HasCeFlags(types.AccImmutable|types.AccPreloaded) {
