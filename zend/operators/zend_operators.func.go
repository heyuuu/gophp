...
func FastEqualCheckFunction(op1 *types.Zval, op2 *types.Zval) bool {
	...
	if op1.IsLong() {
		...
		if op2.IsString() {
			return ZendFastEqualStringsEx(op1.String(), op2.String())
		}
	}
...
func _zendiConvertScalarToNumberEx(op *types.Zval, holder *types.Zval, silent bool) *types.Zval {
	...
	case types.IsString:
		...
		r := zend.StrToNumberEx(op.String(), mode)
		if r.IsInt() {
...
func MulFunction(result *types.Zval, op1 *types.Zval, op2 *types.Zval) int {
	...
	var converted int = 0
	for true {
		var type_pair = TypePair(op1.Type(), op2.Type())
...
func DivFunction(result *types.Zval, op1 *types.Zval, op2 *types.Zval) int {
	...
	var converted int = 0
	for true {
		var type_pair uint = TypePair(op1.Type(), op2.Type())
		if type_pair == TypePair(types.IsLong, types.IsLong) {
			if op2.Long() == 0 {
...
func ConcatFunction(result *types.Zval, op1 *types.Zval, op2 *types.Zval) int {
	...
	if len(op1.String()) == 0 {
		...
		result.SetString(op1.String() + op2.String())
	}
		...
		default:
			...
			if op1.IsObject() {
				if op1.Object().CanGet() {
					...
					tmp_free.SetUndef()
					if op1.Object().Cast(&tmp_free, lang.CondF2(op2.IsFalse() || op2.IsTrue(), types.IsBool, func() __auto__ { return op2.Type() })) == types.FAILURE {
						result.SetLong(1)
...
func IncrementString(str *types.Zval) {
	...
	str.SetString(IncrementStringEx(str.String()))
}
