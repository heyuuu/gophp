...
func ZendDvalToLval(d float64) ZendLong {
	if UNEXPECTED(!(core.ZendFinite(d))) || UNEXPECTED(core.ZendIsnan(d)) {
		return 0
...
func ZvalGetString(op *Zval) *ZendString {
	if EXPECTED(Z_TYPE_P(op) == IS_STRING) {
		return ZendStringCopy(Z_STR_P(op))
	} else {
...
func ZvalTryGetString(op *Zval) *ZendString {
	if EXPECTED(Z_TYPE_P(op) == IS_STRING) {
		var ret *ZendString = ZendStringCopy(Z_STR_P(op))
		return ret
	} else {
...
again:
	...
	case IS_ARRAY:
		...
		break
	case IS_OBJECT:
		if EXPECTED(Z_OBJ_HT_P(op).GetCastObject() == ZendStdCastObjectTostring) {
			result = 1
		} else {
		...
		break
	case IS_RESOURCE:
		if EXPECTED(Z_RES_HANDLE_P(op) != 0) {
			result = 1
		}
		break
	case IS_REFERENCE:
...
const ZVAL_OFFSETOF_TYPE = offsetof(zval, u1.type_info) - offsetof(zval, value)
const ZEND_USE_ASM_ARITHMETIC = 1
...
func FastLongAddFunction(result *Zval, op1 *Zval, op2 *Zval) {
	...
	if UNEXPECTED((Z_LVAL_P(op1)&LONG_SIGN_MASK) == (Z_LVAL_P(op2)&LONG_SIGN_MASK) && (Z_LVAL_P(op1)&LONG_SIGN_MASK) != (Z_LVAL_P(op1)+Z_LVAL_P(op2)&LONG_SIGN_MASK)) {
		ZVAL_DOUBLE(result, float64(Z_LVAL_P(op1)+float64(Z_LVAL_P(op2))))
	} else {
...
func FastAddFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	if EXPECTED(Z_TYPE_P(op1) == IS_LONG) {
		if EXPECTED(Z_TYPE_P(op2) == IS_LONG) {
			...
			ZVAL_DOUBLE(result, float64(Z_LVAL_P(op1))+Z_DVAL_P(op2))
			return SUCCESS
		}
		...
		if EXPECTED(Z_TYPE_P(op2) == IS_DOUBLE) {
			...
			ZVAL_DOUBLE(result, Z_DVAL_P(op1)+float64(Z_LVAL_P(op2)))
			return SUCCESS
		}
	}
...
func FastEqualCheckFunction(op1 *Zval, op2 *Zval) int {
	...
	if EXPECTED(Z_TYPE_P(op1) == IS_LONG) {
		if EXPECTED(Z_TYPE_P(op2) == IS_LONG) {
			...
			return float64(Z_LVAL_P(op1)) == Z_DVAL_P(op2)
		}
	} else if EXPECTED(Z_TYPE_P(op1) == IS_DOUBLE) {
		if EXPECTED(Z_TYPE_P(op2) == IS_DOUBLE) {
			return Z_DVAL_P(op1) == Z_DVAL_P(op2)
		} else if EXPECTED(Z_TYPE_P(op2) == IS_LONG) {
			return Z_DVAL_P(op1) == float64(Z_LVAL_P(op2))
		}
	} else if EXPECTED(Z_TYPE_P(op1) == IS_STRING) {
		if EXPECTED(Z_TYPE_P(op2) == IS_STRING) {
			return ZendFastEqualStrings(Z_STR_P(op1), Z_STR_P(op2))
		}
	}
...
func ConvertObjectToType(op *Zval, dst *Zval, ctype int, conv_func func(op *Zval)) {
	...
	if Z_OBJ_HT_P(op).GetCastObject() != nil {
		if Z_OBJ_HT_P(op).GetCastObject()(op, dst, ctype) == FAILURE {
			ZendError(E_RECOVERABLE_ERROR, "Object of class %s could not be converted to %s", ZSTR_VAL(Z_OBJCE_P(op).GetName()), ZendGetTypeByConst(ctype))
		}
...
try_again:
	...
	case IS_STRING:
		var str *ZendString
		str = Z_STR_P(op)
		if b.Assign(&(Z_TYPE_INFO_P(op)), IsNumericString(ZSTR_VAL(str), ZSTR_LEN(str), &Z_LVAL_P(op), &Z_DVAL_P(op), b.Cond(silent != 0, 1, -1))) == 0 {
			ZVAL_LONG(op, 0)
			if silent == 0 {
	...
	case IS_OBJECT:
		...
		if check != 0 && UNEXPECTED(ExecutorGlobals.GetException() != nil) {
			return
		}
		ZvalPtrDtor(op)
...
func _zendiConvertScalarToNumberEx(op *Zval, holder *Zval, silent ZendBool) *Zval {
	...
	case IS_TRUE:
		...
		return holder
	case IS_STRING:
		if b.Assign(&(Z_TYPE_INFO_P(holder)), IsNumericString(Z_STRVAL_P(op), Z_STRLEN_P(op), &Z_LVAL_P(holder), &Z_DVAL_P(holder), b.Cond(silent != 0, 1, -1))) == 0 {
			ZVAL_LONG(holder, 0)
			if silent == 0 {
				ZendError(E_WARNING, "A non-numeric value encountered")
			}
	...
	case IS_OBJECT:
		...
		if UNEXPECTED(ExecutorGlobals.GetException() != nil) || UNEXPECTED(Z_TYPE_P(holder) != IS_LONG && Z_TYPE_P(holder) != IS_DOUBLE) {
			ZVAL_LONG(holder, 1)
		}
		return holder
	case IS_LONG:
...
try_again:
	...
	case IS_LONG:
		break
	case IS_DOUBLE:
		ZVAL_LONG(op, ZendDvalToLval(Z_DVAL_P(op)))
		break
	case IS_STRING:
		...
		if base == 10 {
			...
			ZVAL_LONG(op, ZEND_STRTOL(ZSTR_VAL(str), nil, base))
		}
		ZendStringReleaseEx(str, 0)
		break
	case IS_ARRAY:
...
try_again:
	...
	case IS_RESOURCE:
		...
		break
	case IS_LONG:
		ZVAL_DOUBLE(op, float64(Z_LVAL_P(op)))
		break
	case IS_DOUBLE:
		break
	case IS_STRING:
		var str *ZendString = Z_STR_P(op)
		ZVAL_DOUBLE(op, ZendStrtod(ZSTR_VAL(str), nil))
		ZendStringReleaseEx(str, 0)
		break
	case IS_ARRAY:
		if ZendHashNumElements(Z_ARRVAL_P(op)) {
...
try_again:
	...
	case IS_RESOURCE:
		...
		break
	case IS_LONG:
		ZVAL_BOOL(op, b.Cond(Z_LVAL_P(op) != 0, 1, 0))
		break
	case IS_DOUBLE:
		ZVAL_BOOL(op, b.Cond(Z_DVAL_P(op), 1, 0))
		break
	case IS_STRING:
		var str *ZendString = Z_STR_P(op)
		if ZSTR_LEN(str) == 0 || ZSTR_LEN(str) == 1 && ZSTR_VAL(str)[0] == '0' {
			ZVAL_FALSE(op)
		} else {
			ZVAL_TRUE(op)
		}
...
try_again:
	...
	case IS_RESOURCE:
		...
		break
	case IS_LONG:
		ZVAL_STR(op, ZendLongToStr(Z_LVAL_P(op)))
		break
	case IS_DOUBLE:
		var str *ZendString
		var dval float64 = Z_DVAL_P(op)
	...
	case IS_OBJECT:
		...
		if ExecutorGlobals.GetException() == nil {
			ZendThrowError(nil, "Object of class %s could not be converted to string", ZSTR_VAL(Z_OBJCE_P(op).GetName()))
		}
		ZvalPtrDtor(op)
		ZVAL_EMPTY_STRING(op)
		break
...
try_again:
	...
	case IS_LONG:
		return Z_LVAL_P(op)
	case IS_DOUBLE:
		return ZendDvalToLval(Z_DVAL_P(op))
	case IS_STRING:
		var type_ ZendUchar
		var lval ZendLong
		var dval float64
		if 0 == b.Assign(&type_, IsNumericString(Z_STRVAL_P(op), Z_STRLEN_P(op), &lval, &dval, b.Cond(silent != 0, 1, -1))) {
			...
			return 0
		} else if EXPECTED(type_ == IS_LONG) {
			return lval
		} else {
...
try_again:
	...
	case IS_LONG:
		return ZendLongToStr(Z_LVAL_P(op))
	case IS_DOUBLE:
		return ZendStrpprintf(0, "%.*G", int(ExecutorGlobals.GetPrecision()), Z_DVAL_P(op))
	case IS_ARRAY:
		ZendError(E_NOTICE, "Array to string conversion")
		if try != 0 && UNEXPECTED(ExecutorGlobals.GetException() != nil) {
			return nil
		} else {
...
func AddFunctionArray(result *Zval, op1 *Zval, op2 *Zval) {
	...
	if result != op1 {
		ZVAL_ARR(result, ZendArrayDup(Z_ARR_P(op1)))
	} else {
	...
	for true {
		if Z_ISREF_P(op1) {
			...
			if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
				...
				if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_ADD, result, op1, op2)) {
					return SUCCESS
				}
			} else if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_ADD, result, op1, op2)) {
				return SUCCESS
			}
			if EXPECTED(op1 != op2) {
	...
	for true {
		if Z_ISREF_P(op1) {
			...
			if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
				...
				if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_SUB, result, op1, op2)) {
					return SUCCESS
				}
			} else if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_SUB, result, op1, op2)) {
				return SUCCESS
			}
			if EXPECTED(op1 != op2) {
				op1 = ZendiConvertScalarToNumber(op1, &op1_copy, result, 0)
		...
		if EXPECTED(type_pair == TYPE_PAIR(IS_LONG, IS_LONG)) {
			...
			if Z_ISREF_P(op1) {
				...
				if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
					...
					if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_MUL, result, op1, op2)) {
						return SUCCESS
					}
				} else if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_MUL, result, op1, op2)) {
					return SUCCESS
				}
				if EXPECTED(op1 != op2) {
					op1 = ZendiConvertScalarToNumber(op1, &op1_copy, result, 0)
...
func PowFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	for true {
		...
		if EXPECTED(type_pair == TYPE_PAIR(IS_LONG, IS_LONG)) {
			if Z_LVAL_P(op2) >= 0 {
				var l1 ZendLong = 1
				var l2 ZendLong = Z_LVAL_P(op1)
				var i ZendLong = Z_LVAL_P(op2)
				if i == 0 {
					ZVAL_LONG(result, 1)
					return SUCCESS
				} else if l2 == 0 {
				...
				ZVAL_DOUBLE(result, pow(float64(Z_LVAL_P(op1)), float64(Z_LVAL_P(op2))))
			}
			return SUCCESS
		} else if EXPECTED(type_pair == TYPE_PAIR(IS_DOUBLE, IS_DOUBLE)) {
			...
			ZVAL_DOUBLE(result, pow(float64(Z_LVAL_P(op1)), Z_DVAL_P(op2)))
			return SUCCESS
		} else if EXPECTED(type_pair == TYPE_PAIR(IS_DOUBLE, IS_LONG)) {
			ZVAL_DOUBLE(result, pow(Z_DVAL_P(op1), float64(Z_LVAL_P(op2))))
			return SUCCESS
		} else {
			if Z_ISREF_P(op1) {
				op1 = Z_REFVAL_P(op1)
				...
				if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
					...
					if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_POW, result, op1, op2)) {
						return SUCCESS
					}
				} else if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_POW, result, op1, op2)) {
					return SUCCESS
				}
				if EXPECTED(op1 != op2) {
	...
	for true {
		...
		if EXPECTED(type_pair == TYPE_PAIR(IS_LONG, IS_LONG)) {
			...
			if Z_LVAL_P(op1)%Z_LVAL_P(op2) == 0 {
				...
				ZVAL_DOUBLE(result, float64(Z_LVAL_P(op1))/Z_LVAL_P(op2))
			}
			return SUCCESS
		} else if EXPECTED(type_pair == TYPE_PAIR(IS_DOUBLE, IS_DOUBLE)) {
			if Z_DVAL_P(op2) == 0 {
			...
			if Z_ISREF_P(op1) {
				...
				if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
					...
					if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_DIV, result, op1, op2)) {
						return SUCCESS
					}
				} else if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_DIV, result, op1, op2)) {
					return SUCCESS
				}
				if EXPECTED(op1 != op2) {
					op1 = ZendiConvertScalarToNumber(op1, &op1_copy, result, 0)
...
func ModFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op1) != IS_LONG) {
			...
			if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
				...
				if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_MOD, result, op1, op2)) {
					return SUCCESS
				}
			}
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op2) != IS_LONG) {
			...
			if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_MOD, result, op1, op2)) {
				return SUCCESS
			}
...
func BooleanXorFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	for {
		if Z_TYPE_P(op1) == IS_FALSE {
			...
			if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
				...
				if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_BOOL_XOR, result, op1, op2)) {
					return SUCCESS
				}
			}
	...
	for {
		if Z_TYPE_P(op2) == IS_FALSE {
			...
			if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_BOOL_XOR, result, op1, op2)) {
				return SUCCESS
			}
...
func BooleanNotFunction(result *Zval, op1 *Zval) int {
	if Z_TYPE_P(op1) < IS_TRUE {
		...
		if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op1, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_BOOL_NOT, result, op1, nil)) {
			return SUCCESS
		}
...
try_again:
	...
	case IS_LONG:
		...
		return SUCCESS
	case IS_DOUBLE:
		ZVAL_LONG(result, ^(ZendDvalToLval(Z_DVAL_P(op1))))
		return SUCCESS
	case IS_STRING:
		var i int
		if Z_STRLEN_P(op1) == 1 {
	...
	case IS_REFERENCE:
		...
		goto try_again
	default:
		if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op1, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_BW_NOT, result, op1, nil)) {
			return SUCCESS
		}
		if result != op1 {
			ZVAL_UNDEF(result)
...
func BitwiseOrFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	if UNEXPECTED(Z_TYPE_P(op1) != IS_LONG) {
		if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
			...
			if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_BW_OR, result, op1, op2)) {
				return SUCCESS
			}
		}
		op1_lval = _zvalGetLongFuncNoisy(op1)
	...
	if UNEXPECTED(Z_TYPE_P(op2) != IS_LONG) {
		if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_BW_OR, result, op1, op2)) {
			return SUCCESS
		}
		op2_lval = _zvalGetLongFuncNoisy(op2)
		if UNEXPECTED(ExecutorGlobals.GetException() != nil) {
...
func BitwiseAndFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	if UNEXPECTED(Z_TYPE_P(op1) != IS_LONG) {
		if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
			...
			if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_BW_AND, result, op1, op2)) {
				return SUCCESS
			}
		}
		op1_lval = _zvalGetLongFuncNoisy(op1)
	...
	if UNEXPECTED(Z_TYPE_P(op2) != IS_LONG) {
		if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_BW_AND, result, op1, op2)) {
			return SUCCESS
		}
		op2_lval = _zvalGetLongFuncNoisy(op2)
		if UNEXPECTED(ExecutorGlobals.GetException() != nil) {
...
func BitwiseXorFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	if UNEXPECTED(Z_TYPE_P(op1) != IS_LONG) {
		if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
			...
			if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_BW_XOR, result, op1, op2)) {
				return SUCCESS
			}
		}
		op1_lval = _zvalGetLongFuncNoisy(op1)
	...
	if UNEXPECTED(Z_TYPE_P(op2) != IS_LONG) {
		if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_BW_XOR, result, op1, op2)) {
			return SUCCESS
		}
		op2_lval = _zvalGetLongFuncNoisy(op2)
		if UNEXPECTED(ExecutorGlobals.GetException() != nil) {
...
func ShiftLeftFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op1) != IS_LONG) {
			...
			if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
				...
				if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_SL, result, op1, op2)) {
					return SUCCESS
				}
			}
			op1_lval = _zvalGetLongFuncNoisy(op1)
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op2) != IS_LONG) {
			...
			if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_SL, result, op1, op2)) {
				return SUCCESS
			}
			op2_lval = _zvalGetLongFuncNoisy(op2)
			if UNEXPECTED(ExecutorGlobals.GetException() != nil) {
	...
	if UNEXPECTED(ZendUlong(op2_lval >= SIZEOF_ZEND_LONG*8) != 0) {
...
func ShiftRightFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op1) != IS_LONG) {
			...
			if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
				...
				if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_SR, result, op1, op2)) {
					return SUCCESS
				}
			}
			op1_lval = _zvalGetLongFuncNoisy(op1)
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op2) != IS_LONG) {
			...
			if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_SR, result, op1, op2)) {
				return SUCCESS
			}
			op2_lval = _zvalGetLongFuncNoisy(op2)
			if UNEXPECTED(ExecutorGlobals.GetException() != nil) {
...
func ConcatFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op1) != IS_STRING) {
			...
			if UNEXPECTED(Z_TYPE_P(op1) == IS_OBJECT) && op1 == result && UNEXPECTED(Z_OBJ_HANDLER_P(op1, get)) && EXPECTED(Z_OBJ_HANDLER_P(op1, set)) {
				...
				if EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op1, do_operation)(ZEND_CONCAT, result, op1, op2)) {
					return SUCCESS
				}
			} else if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_CONCAT, result, op1, op2)) {
				return SUCCESS
			}
	...
	for {
		if UNEXPECTED(Z_TYPE_P(op2) != IS_STRING) {
			...
			if UNEXPECTED(Z_TYPE_P(op2) == IS_OBJECT) && UNEXPECTED(Z_OBJ_HANDLER_P(op2, do_operation)) && EXPECTED(SUCCESS == Z_OBJ_HANDLER_P(op2, do_operation)(ZEND_CONCAT, result, op1, op2)) {
				return SUCCESS
			}
	...
	if UNEXPECTED(Z_STRLEN_P(op1) == 0) {
		...
		if result == op1 && Z_REFCOUNTED_P(result) {
			...
			result_str = ZendStringExtend(Z_STR_P(result), result_len, 0)
...
func StringCompareFunctionEx(op1 *Zval, op2 *Zval, case_insensitive ZendBool) int {
	...
	if case_insensitive != 0 {
		ret = ZendBinaryStrcasecmpL(ZSTR_VAL(str1), ZSTR_LEN(str1), ZSTR_VAL(str2), ZSTR_LEN(str1))
	} else {
		ret = ZendBinaryStrcmp(ZSTR_VAL(str1), ZSTR_LEN(str1), ZSTR_VAL(str2), ZSTR_LEN(str2))
	}
...
func StringCompareFunction(op1 *Zval, op2 *Zval) int {
	if EXPECTED(Z_TYPE_P(op1) == IS_STRING) && EXPECTED(Z_TYPE_P(op2) == IS_STRING) {
		...
		var str1 *ZendString = ZvalGetTmpString(op1, &tmp_str1)
		var str2 *ZendString = ZvalGetTmpString(op2, &tmp_str2)
		var ret int = ZendBinaryStrcmp(ZSTR_VAL(str1), ZSTR_LEN(str1), ZSTR_VAL(str2), ZSTR_LEN(str2))
		ZendTmpStringRelease(tmp_str1)
		ZendTmpStringRelease(tmp_str2)
		return ret
	}
...
func StringCaseCompareFunction(op1 *Zval, op2 *Zval) int {
	if EXPECTED(Z_TYPE_P(op1) == IS_STRING) && EXPECTED(Z_TYPE_P(op2) == IS_STRING) {
		...
		var str1 *ZendString = ZvalGetTmpString(op1, &tmp_str1)
		var str2 *ZendString = ZvalGetTmpString(op2, &tmp_str2)
		var ret int = ZendBinaryStrcasecmpL(ZSTR_VAL(str1), ZSTR_LEN(str1), ZSTR_VAL(str2), ZSTR_LEN(str1))
		ZendTmpStringRelease(tmp_str1)
		ZendTmpStringRelease(tmp_str2)
		return ret
	}
...
func StringLocaleCompareFunction(op1 *Zval, op2 *Zval) int {
	...
	var str1 *ZendString = ZvalGetTmpString(op1, &tmp_str1)
	var str2 *ZendString = ZvalGetTmpString(op2, &tmp_str2)
	var ret int = strcoll(ZSTR_VAL(str1), ZSTR_VAL(str2))
	ZendTmpStringRelease(tmp_str1)
	ZendTmpStringRelease(tmp_str2)
	return ret
}
...
func CompareFunction(result *Zval, op1 *Zval, op2 *Zval) int {
	...
	for true {
		...
		case TYPE_PAIR(IS_LONG, IS_LONG):
			ZVAL_LONG(result, b.CondF2(Z_LVAL_P(op1) > Z_LVAL_P(op2), 1, func() int {
		...
		case TYPE_PAIR(IS_DOUBLE, IS_LONG):
			Z_DVAL_P(result) = Z_DVAL_P(op1) - float64(Z_LVAL_P(op2))
			ZVAL_LONG(result, ZEND_NORMALIZE_BOOL(Z_DVAL_P(result)))
			return SUCCESS
		case TYPE_PAIR(IS_LONG, IS_DOUBLE):
			Z_DVAL_P(result) = float64(Z_LVAL_P(op1) - Z_DVAL_P(op2))
			ZVAL_LONG(result, ZEND_NORMALIZE_BOOL(Z_DVAL_P(result)))
			return SUCCESS
		...
		case TYPE_PAIR(IS_STRING, IS_STRING):
			...
			ZVAL_LONG(result, ZendiSmartStrcmp(Z_STR_P(op1), Z_STR_P(op2)))
			return SUCCESS
		...
		default:
			...
			if Z_TYPE_P(op1) == IS_OBJECT {
				if Z_OBJ_HT_P(op1).GetGet() != nil {
					...
					return ret
				} else if Z_TYPE_P(op2) != IS_OBJECT && Z_OBJ_HT_P(op1).GetCastObject() != nil {
					ZVAL_UNDEF(&tmp_free)
					if Z_OBJ_HT_P(op1).GetCastObject()(op1, &tmp_free, b.CondF2(Z_TYPE_P(op2) == IS_FALSE || Z_TYPE_P(op2) == IS_TRUE, _IS_BOOL, func() ZendUchar { return Z_TYPE_P(op2) })) == FAILURE {
						ZVAL_LONG(result, 1)
			...
			if Z_TYPE_P(op2) == IS_OBJECT {
				if Z_OBJ_HT_P(op2).GetGet() != nil {
					...
					ZendFreeObjGetResult(op_free)
					return ret
				} else if Z_TYPE_P(op1) != IS_OBJECT && Z_OBJ_HT_P(op2).GetCastObject() != nil {
					ZVAL_UNDEF(&tmp_free)
					if Z_OBJ_HT_P(op2).GetCastObject()(op2, &tmp_free, b.CondF2(Z_TYPE_P(op1) == IS_FALSE || Z_TYPE_P(op1) == IS_TRUE, _IS_BOOL, func() ZendUchar { return Z_TYPE_P(op1) })) == FAILURE {
						ZVAL_LONG(result, -1)
						ZendFreeObjGetResult(&tmp_free)
						return SUCCESS
					}
...
func IncrementString(str *Zval) {
	...
	if !(Z_REFCOUNTED_P(str)) {
		Z_STR_P(str) = ZendStringInit(Z_STRVAL_P(str), Z_STRLEN_P(str), 0)
...
func ZendObjectIsTrue(op *Zval) int {
	if Z_OBJ_HT_P(op).GetCastObject() != nil {
		...
		ZendError(E_RECOVERABLE_ERROR, "Object of class %s could not be converted to bool", ZSTR_VAL(Z_OBJ_P(op).GetCe().GetName()))
	} else if Z_OBJ_HT_P(op).GetGet() != nil {
		var result int
		var rv Zval
...
func ZendStringTolowerEx(str *ZendString, persistent int) *ZendString {
	var p *uint8 = (*uint8)(ZSTR_VAL(str))
	var end *uint8 = p + ZSTR_LEN(str)
	for p < end {
		if (*p) != ZendTolowerAscii(*p) {
			var res *ZendString = ZendStringAlloc(ZSTR_LEN(str), persistent)
			var r *uint8
			if p != (*uint8)(ZSTR_VAL(str)) {
				memcpy(ZSTR_VAL(res), ZSTR_VAL(str), p-(*uint8)(ZSTR_VAL(str)))
			}
...
func ZendBinaryZvalStrncmp(s1 *Zval, s2 *Zval, s3 *Zval) int {
	return ZendBinaryStrncmp(Z_STRVAL_P(s1), Z_STRLEN_P(s1), Z_STRVAL_P(s2), Z_STRLEN_P(s2), Z_LVAL_P(s3))
}
...
func ZendBinaryZvalStrncasecmp(s1 *Zval, s2 *Zval, s3 *Zval) int {
	return ZendBinaryStrncasecmpL(Z_STRVAL_P(s1), Z_STRLEN_P(s1), Z_STRVAL_P(s2), Z_STRLEN_P(s2), Z_LVAL_P(s3))
}
...
func ZendLocaleSprintfDouble(op *Zval) {
	var str *ZendString
	str = ZendStrpprintf(0, "%.*G", int(ExecutorGlobals.GetPrecision()), float64(Z_DVAL_P(op)))
	ZVAL_NEW_STR(op, str)
}
...
func IsNumericStrFunction(str *ZendString, lval *ZendLong, dval *float64) ZendUchar {
	return IsNumericStringEx(ZSTR_VAL(str), ZSTR_LEN(str), lval, dval, -1, nil)
}
