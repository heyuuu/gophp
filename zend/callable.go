	...
	if gotoGetFunctionViaHandler {
		if fcc.GetObject() != nil && fcc.GetCallingScope() == ceOrg {
			...
			if fcc.GetCallingScope().GetGetStaticMethod() != nil {
				fcc.SetFunctionHandler(fcc.GetCallingScope().GetGetStaticMethod()(fcc.GetCallingScope(), methodName))
			} else {
			...
			if fcc.GetFunctionHandler().IsAbstract() {
				...
				if !throw {
					...
					if severity == faults.E_ERROR {
						faults.ThrowError(nil, fmt.Sprintf("Non-static method %s::%s() %s be called statically", fcc.GetCallingScope().Name(), fcc.GetFunctionHandler().FunctionName(), verb))
					} else {
