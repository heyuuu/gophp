...
func ZEND_REF_HAS_TYPE_SOURCES(ref *types2.ZendReference) bool {
	return ref.GetSources().GetPtr() != nil
}
func ZEND_REF_FIRST_SOURCE(ref *types2.ZendReference) *ZendPropertyInfo {
	if types2.ZEND_PROPERTY_INFO_SOURCE_IS_LIST(ref.GetSources().GetList()) != 0 {
		return types2.ZEND_PROPERTY_INFO_SOURCE_TO_LIST(ref.GetSources().GetList()).GetPtr()[0]
	} else {
		return ref.GetSources().GetPtr()
	}
}
func ZendCopyToVariable(variable_ptr *types2.Zval, value *types2.Zval) {
	variable_ptr.CopyValueFrom(value)
}
func ZendAssignToVariable(variable_ptr *types2.Zval, value *types2.Zval, value_type types2.ZendUchar, strict types2.ZendBool) *types2.Zval {
	var ref *types2.ZendRefcounted = nil
	...
	for {
		if variable_ptr.IsRefcounted() {
			var garbage *types2.ZendRefcounted
			...
			if variable_ptr.IsObject() && types2.Z_OBJ_HT(*variable_ptr).GetSet() != nil {
				types2.Z_OBJ_HT(*variable_ptr).GetSet()(variable_ptr, value)
				return variable_ptr
			}
...
func ZendVmStackFreeExtraArgsEx(call_info uint32, call *ZendExecuteData) {
	if (call_info & ZEND_CALL_FREE_EXTRA_ARGS) != 0 {
		...
		var p *types2.Zval = call.VarNum(call.GetFunc().GetOpArray().GetLastVar() + call.GetFunc().GetOpArray().GetT())
		for {
			if p.IsRefcounted() {
				var r *types2.ZendRefcounted = p.RefCounted()
				if r.DelRefcount() == 0 {
					p.SetNull()
					//RcDtorFunc(r)
				} else {
...
func CACHE_POLYMORPHIC_PTR_EX(slot *any, ce *types2.ClassEntry, ptr any) {
	slot[0] = ce
	slot[1] = ptr
}
...
func ZEND_CLASS_HAS_TYPE_HINTS(ce *types2.ClassEntry) bool {
	return (ce.GetCeFlags() & AccHasTypeHints) == AccHasTypeHints
}
func ZEND_REF_ADD_TYPE_SOURCE(ref *types2.ZendReference, source *ZendPropertyInfo) {
	ZendRefAddTypeSource(&(ref.GetSources()), source)
}
func ZEND_REF_DEL_TYPE_SOURCE(ref *types2.ZendReference, source *ZendPropertyInfo) {
	ZendRefDelTypeSource(&(ref.GetSources()), source)
}
func GetZvalPtr(op_type int, node ZnodeOp, should_free *ZendFreeOp, type_ int) *types2.Zval {
	return _getZvalPtr(op_type, node, should_free, type_, executeData, opline)
}
func GetZvalPtrUndef(op_type int, node ZnodeOp, should_free *ZendFreeOp, type_ int) *types2.Zval {
	return _getZvalPtrUndef(op_type, node, should_free, type_, executeData, opline)
}
func GetOpDataZvalPtrR(op_type int, node ZnodeOp, should_free *ZendFreeOp) *types2.Zval {
	return _getOpDataZvalPtrR(op_type, node, should_free, executeData, opline)
}
func GetZvalPtrPtr(op_type int, node ZnodeOp, should_free *ZendFreeOp, type_ int) *types2.Zval {
...
func ZifPass(executeData *ZendExecuteData, return_value *types2.Zval) {}
func FREE_VAR_PTR_AND_EXTRACT_RESULT_IF_NECESSARY(free_op *types2.Zval, result *types2.Zval) {
	var __container_to_free *types2.Zval = free_op
	if __container_to_free != nil && __container_to_free.IsRefcounted() {
		var __ref *types2.ZendRefcounted = __container_to_free.RefCounted()
		if __ref.DelRefcount() == 0 {
			var __zv *types2.Zval = result
			if __zv.IsIndirect() {
				types2.ZVAL_COPY(__zv, __zv.Indirect())
			}
			//RcDtorFunc(__ref)
		}
	}
...
func _getZvalPtrTmp(var_ uint32, should_free *ZendFreeOp, executeData *ZendExecuteData) *types2.Zval {
	var ret *types2.Zval = EX_VAR(executeData, var_)
	*should_free = ret
	b.Assert(ret.GetType() != types2.IS_REFERENCE)
	return ret
}
func _getZvalPtrVar(var_ uint32, should_free *ZendFreeOp, executeData *ZendExecuteData) *types2.Zval {
	var ret *types2.Zval = EX_VAR(executeData, var_)
	*should_free = ret
	return ret
}
func _getZvalPtrVarDeref(var_ uint32, should_free *ZendFreeOp, executeData *ZendExecuteData) *types2.Zval {
	var ret *types2.Zval = EX_VAR(executeData, var_)
	*should_free = ret
	ret = types2.ZVAL_DEREF(ret)
	return ret
}
func ZvalUndefinedCv(var_ uint32, executeData *ZendExecuteData) *types2.Zval {
	if EG__().GetException() == nil {
		var cv *types2.String = CV_DEF_OF(EX_VAR_TO_NUM(var_))
		faults.Error(faults.E_NOTICE, "Undefined variable: %s", cv.GetVal())
	}
	return EG__().GetUninitializedZval()
}
func _zvalUndefinedOp1(executeData *ZendExecuteData) *types2.Zval {
	return ZvalUndefinedCv(executeData.GetOpline().op1.var_, executeData)
}
func _zvalUndefinedOp2(executeData *ZendExecuteData) *types2.Zval {
	return ZvalUndefinedCv(executeData.GetOpline().op2.var_, executeData)
}
func ZVAL_UNDEFINED_OP1(executeData *ZendExecuteData) *types2.Zval {
	return _zvalUndefinedOp1(executeData)
}
func ZVAL_UNDEFINED_OP2(executeData *ZendExecuteData) *types2.Zval {
	return _zvalUndefinedOp2(executeData)
}
func _getZvalCvLookup(ptr *types2.Zval, var_ uint32, type_ int, executeData *ZendExecuteData) *types2.Zval {
	switch type_ {
	case BP_VAR_R:
		fallthrough
	case BP_VAR_UNSET:
...
func _getZvalPtrCv(var_ uint32, type_ int, executeData *ZendExecuteData) *types2.Zval {
	var ret *types2.Zval = EX_VAR(executeData, var_)
	if ret.IsUndef() {
		if type_ == BP_VAR_W {
			ret.SetNull()
...
func _getZvalPtrCvDeref(var_ uint32, type_ int, executeData *ZendExecuteData) *types2.Zval {
	var ret *types2.Zval = EX_VAR(executeData, var_)
	if ret.IsUndef() {
		if type_ == BP_VAR_W {
			ret.SetNull()
...
func _get_zval_ptr_cv_BP_VAR_R(var_ uint32, executeData *ZendExecuteData) *types2.Zval {
	var ret *types2.Zval = EX_VAR(executeData, var_)
	if ret.IsUndef() {
...
func _get_zval_ptr_cv_deref_BP_VAR_R(var_ uint32, executeData *ZendExecuteData) *types2.Zval {
	var ret *types2.Zval = EX_VAR(executeData, var_)
	if ret.IsUndef() {
		return ZvalUndefinedCv(var_, executeData)
	}
