...
func AddIndexZval(arg *Zval, index ZendUlong, value *Zval) int {
	if Z_ARRVAL_P(arg).IndexUpdate(index, value) != nil {
		return SUCCESS
	} else {
		return FAILURE
	}
...
func AddNextIndexZval(arg *Zval, value *Zval) int {
	if Z_ARRVAL_P(arg).NextIndexInsert(value) != nil {
		return SUCCESS
	} else {
		return FAILURE
	}
...
func ZendParseArgPathStr(arg *Zval, dest **ZendString, check_null int) int {
	if ZendParseArgStr(arg, dest, check_null) == 0 || (*dest) != nil && CHECK_NULL_PATH(ZSTR_VAL(*dest), ZSTR_LEN(*dest)) {
		return 0
	}
	return 1
}
...
func ZendParseArgArray(arg *Zval, dest **Zval, check_null int, or_object int) int {
	if Z_TYPE_P(arg) == IS_ARRAY || or_object != 0 && Z_TYPE_P(arg) == IS_OBJECT {
		*dest = arg
	} else if check_null != 0 && Z_TYPE_P(arg) == IS_NULL {
		*dest = nil
	} else {
...
func ZendParseArgArrayHt(arg *Zval, dest **HashTable, check_null int, or_object int, separate int) int {
	if Z_TYPE_P(arg) == IS_ARRAY {
		*dest = Z_ARRVAL_P(arg)
	} else if or_object != 0 && Z_TYPE_P(arg) == IS_OBJECT {
...
func ZendParseArgObject(arg *Zval, dest **Zval, ce *ZendClassEntry, check_null int) int {
	if Z_TYPE_P(arg) == IS_OBJECT && (ce == nil || InstanceofFunction(Z_OBJCE_P(arg), ce) != 0) {
		*dest = arg
	} else if check_null != 0 && Z_TYPE_P(arg) == IS_NULL {
		*dest = nil
	} else {
...
func ZendParseArgZval(arg *Zval, dest **Zval, check_null int) {
	if check_null != 0 && (Z_TYPE_P(arg) == IS_NULL || Z_ISREF_P(arg) && Z_TYPE_P(Z_REFVAL_P(arg)) == IS_NULL) {
		*dest = nil
	} else {
		*dest = arg
	}
...
func ZendCopyParametersArray(param_count int, argument_array *Zval) int {
	...
	for b.PostDec(&param_count) > 0 {
		...
		Z_ARRVAL_P(argument_array).NextIndexInsertNew(param_ptr)
		param_ptr++
	}
	return SUCCESS
}
...
func ZendParseArgLongWeak(arg *Zval, dest *ZendLong) int {
	if Z_TYPE_P(arg) == IS_DOUBLE {
		if core.ZendIsnan(Z_DVAL_P(arg)) {
			return 0
		}
		...
		var d float64
		var type_ int
		if b.Assign(&type_, IsNumericStrFunction(Z_STR_P(arg), dest, &d)) != IS_LONG {
			if type_ != 0 {
				if core.ZendIsnan(d) {
					return 0
				}
...
func ZendParseArgDoubleWeak(arg *Zval, dest *float64) int {
	if Z_TYPE_P(arg) == IS_LONG {
		*dest = float64(Z_LVAL_P(arg))
		...
		var l ZendLong
		var type_ int
		if b.Assign(&type_, IsNumericStrFunction(Z_STR_P(arg), &l, dest)) != IS_DOUBLE {
			if type_ != 0 {
				*dest = float64(l)
			} else {
				return 0
			...
			if key != nil {
				...
				if property_info != ZEND_WRONG_PROPERTY_INFO && property_info != nil && !property_info.IsStatic() {
					...
					if object.GetProperties() != nil {
						...
						object.GetProperties().Update(key, &tmp)
					}
				} else {
					if object.GetProperties() == nil {
						RebuildObjectProperties(object)
					}
					prop = object.GetProperties().Update(key, prop)
					ZvalAddRef(prop)
				}
			} else {
				if object.GetProperties() == nil {
...
func AddIndexLong(arg *Zval, index ZendUlong, n ZendLong) int {
	var tmp Zval
	ZVAL_LONG(&tmp, n)
	Z_ARRVAL_P(arg).IndexUpdate(index, &tmp)
	return SUCCESS
}
...
func AddNextIndexLong(arg *Zval, n ZendLong) int {
	var tmp Zval
	ZVAL_LONG(&tmp, n)
	if Z_ARRVAL_P(arg).NextIndexInsert(&tmp) != nil {
		return SUCCESS
	} else {
		return FAILURE
	}
...
func ArraySetZvalKey(ht *HashTable, key *Zval, value *Zval) int {
	...
	case IS_TRUE:
		...
		break
	case IS_LONG:
		result = ht.IndexUpdate(Z_LVAL_P(key), value)
		break
	case IS_DOUBLE:
		result = ht.IndexUpdate(ZendDvalToLval(Z_DVAL_P(key)), value)
		break
	default:
	...
	if module.GetDeps() != nil {
		...
		for dep.GetName() != nil {
			if dep.GetType() == MODULE_DEP_REQUIRED {
				...
				if b.Assign(&req_mod, &ModuleRegistry.FindPtr(lcname)) == nil || req_mod.GetModuleStarted() == 0 {
...
func ZendStartupModules() int {
	&ModuleRegistry.SortEx(ZendSortModules, nil, 0)
	&ModuleRegistry.Apply(ZendStartupModuleZval)
	return SUCCESS
}
...
func ZendRegisterModuleEx(module *ZendModuleEntry) *ZendModuleEntry {
	...
	if module.GetDeps() != nil {
		...
		for dep.GetName() != nil {
			if dep.GetType() == MODULE_DEP_CONFLICTS {
				...
				lcname = ZendStringAlloc(name_len, 0)
				ZendStrTolowerCopy(ZSTR_VAL(lcname), dep.GetName(), name_len)
				if &ModuleRegistry.Exists(lcname) != 0 || ZendGetExtension(dep.GetName()) != nil {
					ZendStringEfree(lcname)
	...
	if b.Assign(&module_ptr, &ModuleRegistry.AddMem(lcname, module, b.SizeOf("zend_module_entry"))) == nil {
...
func ZendRegisterFunctions(scope *ZendClassEntry, functions *ZendFunctionEntry, function_table *HashTable, type_ int) int {
	...
	for ptr.GetFname() != nil {
		...
		if target_function_table.AddPtr(lowercase_name, reg_function) == nil {
			unload = 1
			Free(reg_function)
	...
	if unload != 0 {
		...
		for ptr.GetFname() != nil {
			...
			if target_function_table.Exists(lowercase_name) != 0 {
...
func ZendUnregisterFunctions(functions *ZendFunctionEntry, count int, function_table *HashTable) {
	...
	for ptr.GetFname() != nil {
		...
		lowercase_name = ZendStringAlloc(fname_len, 0)
		ZendStrTolowerCopy(ZSTR_VAL(lowercase_name), ptr.GetFname(), fname_len)
		target_function_table.Del(lowercase_name)
		ZendStringEfree(lowercase_name)
		ptr++
		i++
	}
...
func ZendGetModuleStarted(module_name *byte) int {
	var module *ZendModuleEntry
	module = &ModuleRegistry.StrFindPtr(module_name, strlen(module_name))
...
func CleanModuleClasses(module_number int) {
	ExecutorGlobals.GetClassTable().ApplyWithArgument(CleanModuleClass, any(&module_number))
}
	...
	if ExecutorGlobals.GetFullTablesCleanup() != 0 {
		...
		for {
			...
			for _idx = __ht.GetNNumUsed(); _idx > 0; _idx-- {
				...
				var i uint32 = __ht.Hash(nIndex)
				if j != i {
					var prev *Bucket = __ht.HashToBucket(i)
					for Z_NEXT(prev.GetVal()) != j {
						i = Z_NEXT(prev.GetVal())
						prev = __ht.HashToBucket(i)
					}
					Z_NEXT(prev.GetVal()) = Z_NEXT(_p.GetVal())
				} else {
					__ht.Hash(nIndex) = Z_NEXT(_p.GetVal())
				}
			}
...
func ZendNextFreeModule() int { return &ModuleRegistry.NumElements() + 1 }
func DoRegisterInternalClass(orig_class_entry *ZendClassEntry, ce_flags uint32) *ZendClassEntry {
	...
	*class_entry = *orig_class_entry
	...
	lowercase_name = ZendNewInternedString(lowercase_name)
	CompilerGlobals.GetClassTable().UpdatePtr(lowercase_name, class_entry)
...
func ZendSetHashSymbol(symbol *Zval, name *byte, name_length int, is_ref ZendBool, num_symbol_tables int, _ ...any) int {
	...
	for b.PostDec(&num_symbol_tables) > 0 {
		...
		symbol_table.StrUpdate(name, name_length, symbol)
...
func ZendDisableFunction(function_name *byte, function_name_length int) int {
	...
	if b.Assign(&func_, CompilerGlobals.GetFunctionTable().StrFindPtr(function_name, function_name_length)) {
...
func ZendDisableClass(class_name *byte, class_name_length int) int {
	...
	disabled_class = CompilerGlobals.GetClassTable().FindPtr(key)
	ZendStringReleaseEx(key, 0)
	...
	if strict_class != 0 && fcc.GetCallingScope() != nil && ZendStringEqualsLiteral(lmname, ZEND_CONSTRUCTOR_FUNC_NAME) {
		...
		if fcc.GetFunctionHandler().GetOpArray().IsChanged() && strict_class == 0 {
			...
			if scope != nil && InstanceofFunction(fcc.GetFunctionHandler().GetScope(), scope) != 0 {
				zv = &scope.GetFunctionTable().Find(lmname)
				if zv != nil {
...
try_again:
	...
	case IS_ARRAY:
		var method *Zval = nil
		var obj *Zval = nil
		if Z_ARRVAL_P(callable).NumElements() == 2 {
			obj = Z_ARRVAL_P(callable).IndexFindDeref(0)
			method = Z_ARRVAL_P(callable).IndexFindDeref(1)
		}
...
again:
	...
	case IS_ARRAY:
		...
		var obj *Zval = nil
		if Z_ARRVAL_P(callable).NumElements() == 2 {
			obj = Z_ARRVAL_P(callable).IndexFind(0)
			method = Z_ARRVAL_P(callable).IndexFind(1)
		}
		for {
	...
	case IS_OBJECT:
		if Z_OBJ_HANDLER_P(callable, get_closure) {
			if Z_OBJ_HANDLER_P(callable, get_closure)(callable, &fcc.GetCallingScope(), &fcc.GetFunctionHandler(), &fcc.GetObject()) == SUCCESS {
				fcc.SetCalledScope(fcc.GetCallingScope())
				if fcc == &fcc_local {
					ZendReleaseFcallInfoCache(fcc)
				}
...
func ZendFcallInfoArgsEx(fci *ZendFcallInfo, func_ *ZendFunction, args *Zval) int {
	...
	fci.SetParamCount(Z_ARRVAL_P(args).NumElements())
...
func ZendGetModuleVersion(module_name *byte) *byte {
	...
	module = &ModuleRegistry.FindPtr(lname)
...
func ZendDeclareTypedProperty(ce *ZendClassEntry, name *ZendString, property *Zval, access_type int, doc_comment *ZendString, type_ ZendType) int {
	...
	if (access_type & ZEND_ACC_STATIC) != 0 {
		if b.Assign(&property_info_ptr, &ce.GetPropertiesInfo().FindPtr(name)) != nil && property_info_ptr.IsStatic() {
			property_info.SetOffset(property_info_ptr.GetOffset())
			ZvalPtrDtor(&ce.GetDefaultStaticMembersTable()[property_info.GetOffset()])
			&ce.GetPropertiesInfo().Del(name)
		} else {
	...
	&ce.GetPropertiesInfo().UpdatePtr(name, property_info)
	return SUCCESS
}
...
func ZendDeclareClassConstantEx(ce *ZendClassEntry, name *ZendString, value *Zval, access_type int, doc_comment *ZendString) int {
	...
	if !(&ce.GetConstantsTable().AddPtr(name, c)) {
