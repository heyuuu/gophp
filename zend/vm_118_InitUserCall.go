...
func ZEND_INIT_USER_CALL_SPEC_CONST_CONST_HANDLER(executeData *ZendExecuteData) int {
	...
	if ZendIsCallableEx(function_name, nil, 0, nil, &fcc, &error) != 0 {
		...
		faults.InternalTypeError(executeData.IsCallUseStrictTypes(), fmt.Sprintf("%s() expects parameter 1 to be a valid callback, %s", opline.Const1().StringEx().GetVal(), error))
		Efree(error)
		if EG__().HasException() {
			return 0
		}
