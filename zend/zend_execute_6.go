...
func ZendThrowRefTypeErrorType(prop1 *types.PropertyInfo, prop2 *types.PropertyInfo, zv *types.Zval) {
	...
	faults.TypeError(fmt.Sprintf("Reference with value of type %s held by property %s::$%s of type %s%s is not compatible with property %s::$%s of type %s%s", lang.CondF(zv.IsObject(), func() []byte { return types.Z_OBJCE_P(zv).Name() }, func() *byte { return types.ZendGetTypeByConst(zv.Type()) }), prop1.GetCe().Name(), ZendGetUnmangledPropertyNameEx(prop1.GetName()), prop1_type1, prop1_type2, prop2.GetCe().Name(), ZendGetUnmangledPropertyNameEx(prop2.GetName()), prop2_type1, prop2_type2))
}
func ZendThrowRefTypeErrorZval(prop *types.PropertyInfo, zv *types.Zval) {
	var prop_type1 *byte
	var prop_type2 *byte
	ZendFormatType(prop.GetType(), &prop_type1, &prop_type2)
	faults.TypeError(fmt.Sprintf("Cannot assign %s to reference held by property %s::$%s of type %s%s", lang.CondF(zv.IsObject(), func() []byte { return types.Z_OBJCE_P(zv).Name() }, func() *byte { return types.ZendGetTypeByConst(zv.Type()) }), prop.GetCe().Name(), ZendGetUnmangledPropertyNameEx(prop.GetName()), prop_type1, prop_type2))
}
func ZendThrowConflictingCoercionError(prop1 *types.PropertyInfo, prop2 *types.PropertyInfo, zv *types.Zval) {
	...
	ZendFormatType(prop1.GetType(), &prop1_type1, &prop1_type2)
	ZendFormatType(prop2.GetType(), &prop2_type1, &prop2_type2)
	faults.TypeError(fmt.Sprintf("Cannot assign %s to reference held by property %s::$%s of type %s%s and property %s::$%s of type %s%s, as this would result in an inconsistent type conversion", lang.CondF(zv.IsObject(), func() []byte { return types.Z_OBJCE_P(zv).Name() }, func() *byte { return types.ZendGetTypeByConst(zv.Type()) }), prop1.GetCe().Name(), ZendGetUnmangledPropertyNameEx(prop1.GetName()), prop1_type1, prop1_type2, prop2.GetCe().Name(), ZendGetUnmangledPropertyNameEx(prop2.GetName()), prop2_type1, prop2_type2))
}
