...
func _zendTsHashInit(ht *TsHashTable, nSize uint32, pDestructor DtorFuncT, persistent ZendBool) {
	TS_HASH(ht)._init(nSize, pDestructor, persistent)
}
...
func ZendTsHashAdd(ht *TsHashTable, key *ZendString, pData *Zval) *Zval {
	var retval *Zval
	BeginWrite(ht)
	retval = TS_HASH(ht).Add(key, pData)
	EndWrite(ht)
	return retval
}
func ZendTsHashUpdate(ht *TsHashTable, key *ZendString, pData *Zval) *Zval {
	var retval *Zval
	BeginWrite(ht)
	retval = TS_HASH(ht).Update(key, pData)
	EndWrite(ht)
	return retval
}
func ZendTsHashNextIndexInsert(ht *TsHashTable, pData *Zval) *Zval {
	var retval *Zval
	BeginWrite(ht)
	retval = TS_HASH(ht).NextIndexInsert(pData)
	EndWrite(ht)
	return retval
}
func ZendTsHashIndexUpdate(ht *TsHashTable, h ZendUlong, pData *Zval) *Zval {
	var retval *Zval
	BeginWrite(ht)
	retval = TS_HASH(ht).IndexUpdate(h, pData)
	EndWrite(ht)
	return retval
}
...
func ZendTsHashMergeEx(target *TsHashTable, source *TsHashTable, pCopyConstructor CopyCtorFuncT, pMergeSource MergeCheckerFuncT, pParam any) {
	BeginRead(source)
	BeginWrite(target)
	TS_HASH(target).MergeEx(TS_HASH(source), pCopyConstructor, pMergeSource, pParam)
	EndWrite(target)
	EndRead(source)
}
func ZendTsHashSort(ht *TsHashTable, sort_func SortFuncT, compare_func CompareFuncT, renumber int) int {
	var retval int
	BeginWrite(ht)
	retval = TS_HASH(ht).SortEx(sort_func, compare_func, renumber)
	EndWrite(ht)
	return retval
}
func ZendTsHashCompare(ht1 *TsHashTable, ht2 *TsHashTable, compar CompareFuncT, ordered ZendBool) int {
	...
	BeginRead(ht1)
	BeginRead(ht2)
	retval = TS_HASH(ht1).Compare(TS_HASH(ht2), compar, ordered)
	EndRead(ht2)
	EndRead(ht1)
	return retval
}
func ZendTsHashMinmax(ht *TsHashTable, compar CompareFuncT, flag int) *Zval {
	var retval *Zval
	BeginRead(ht)
	retval = TS_HASH(ht).Minmax(compar, flag)
	EndRead(ht)
	return retval
}
...
func ZendTsHashStrFind(ht *TsHashTable, key *byte, len_ int) *Zval {
	var retval *Zval
	BeginRead(ht)
	retval = TS_HASH(ht).StrFind(key, len_)
	EndRead(ht)
	return retval
}
func ZendTsHashStrUpdate(ht *TsHashTable, key *byte, len_ int, pData *Zval) *Zval {
	var retval *Zval
	BeginWrite(ht)
	retval = TS_HASH(ht).StrUpdate(key, len_, pData)
	EndWrite(ht)
	return retval
}
func ZendTsHashStrAdd(ht *TsHashTable, key *byte, len_ int, pData *Zval) *Zval {
	var retval *Zval
	BeginWrite(ht)
	retval = TS_HASH(ht).StrAdd(key, len_, pData)
	EndWrite(ht)
	return retval
}
