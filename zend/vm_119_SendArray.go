...
func ZEND_SEND_ARRAY_SPEC_HANDLER(executeData *ZendExecuteData) int {
	...
	if !args.IsArray() {
		if (opline.GetOp1Type()&(IS_VAR|IS_CV)) != 0 && args.IsRef() {
			args = types.Z_REFVAL_P(args)
			if args.IsArray() {
				goto send_array
			}
		}
		faults.InternalTypeError(executeData.IsCallUseStrictTypes(), fmt.Sprintf("call_user_func_array() expects parameter 2 to be array, %s given", types.ZendGetTypeByConst(args.Type())))
			...
			if skip < count && len_ > 0 {
				...
				for _, _p := range __ht.ForeachData() {
					...
					if skip > 0 {
						...
						if arg.IsRef() && (executeData.GetCall().func_.common.fn_flags&types.AccCallViaTrampoline) == 0 {
