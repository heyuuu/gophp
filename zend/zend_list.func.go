...
func ZendResourceDtor(res *types.Resource) {
	...
	if ld != nil {
		...
		faults.Error(faults.E_WARNING, fmt.Sprintf("Unknown list entry type (%d)", r.GetType()))
	}
}
...
func ZendFetchResource2(res *types.Resource, resource_type_name string, resource_type1 int, resource_type2 int) any {
	...
	if resource_type_name {
		faults.Error(faults.E_WARNING, fmt.Sprintf("%s(): supplied resource is not a valid %s resource", CurrEX().CalleeName(), resource_type_name))
	}
	return nil
}
...
func ZendFetchResourceEx(res *types.Zval, resource_type_name string, resource_type int) any {
	if res == nil {
		if resource_type_name {
			faults.Error(faults.E_WARNING, fmt.Sprintf("%s(): no %s resource supplied", CurrEX().CalleeName(), resource_type_name))
		}
		return nil
	}
	if !res.IsResource() {
		if resource_type_name {
			faults.Error(faults.E_WARNING, fmt.Sprintf("%s(): supplied argument is not a valid %s resource", CurrEX().CalleeName(), resource_type_name))
		}
		return nil
	}
...
func PlistEntryDtor(res *types.Resource) {
	if res.GetType() >= 0 {
		...
		if ld != nil {
			...
			faults.Error(faults.E_WARNING, fmt.Sprintf("Unknown list entry type (%d)", res.GetType()))
		}
	}
	Free(res)
}
