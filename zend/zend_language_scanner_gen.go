...
func LexScan(zendlval *Zval, elem *ZendParserStackElem, sc *LangScanner) int {
	var token int
	var offset int
	var start_line int = CG__().GetZendLineno()
	zendlval.SetUndef()
restart:
	LANG_SCNG__().yy_text = YYCURSOR
	var yych uint8
	var yyaccept uint = 0
	if YYGETCONDITION() < 5 {
		if YYGETCONDITION() < 2 {
...
yy1:
	...
	if !(LANG_SCNG__().heredoc_scan_ahead) {
...
yy10:
	...
	for true {
		if YYCURSOR < YYLIMIT {
			...
			Yyleng = YYLIMIT - LANG_SCNG__().yy_text
...
skip_escape_conversion:
	if LANG_SCNG__().output_filter {
		...
		LANG_SCNG__().output_filter((**uint8)(&str), &sz, (*uint8)(s), int(Z_STRLEN_P(zendlval)))
...
yy56:
	...
	Yyleng = YYCURSOR - LANG_SCNG__().yy_text
	RESET_DOC_COMMENT()
	if ZendStackIsEmpty(&(LANG_SCNG__().state_stack)) == 0 {
		YyPopState()
	}
	token = '}'
	goto emit_token
...
yy89:
	...
	if YYCURSOR < YYLIMIT {
		YYCURSOR++
	} else if !(LANG_SCNG__().heredoc_scan_ahead) {
...
yy334:
	...
	ZendPtrStackPush(&(LANG_SCNG__().heredoc_label_stack), any(heredoc_label))
	...
	YYCURSOR = saved_cursor
	if is_heredoc != 0 && !(LANG_SCNG__().heredoc_scan_ahead) {
		var current_state ZendLexState
		var saved_doc_comment *ZendString = CG__().GetDocComment()
		var heredoc_nesting_level int = 1
		var first_token int = 0
		var error int = 0
		ZendSaveLexicalState(&current_state)
		LANG_SCNG__().heredoc_scan_ahead = 1
		LANG_SCNG__().heredoc_indentation = 0
		LANG_SCNG__().heredoc_indentation_uses_spaces = 0
		LANG_SCNG__().on_event = nil
		CG__().SetDocComment(nil)
		...
		if (first_token == T_VARIABLE || first_token == T_DOLLAR_OPEN_CURLY_BRACES || first_token == T_CURLY_OPEN) && LANG_SCNG__().heredoc_indentation {
			ZendThrowExceptionEx(ZendCeParseError, 0, "Invalid body indentation level (expecting an indentation level of at least %d)", LANG_SCNG__().heredoc_indentation)
			error = 1
		}
		heredoc_label.SetIndentation(LANG_SCNG__().heredoc_indentation)
		heredoc_label.SetIndentationUsesSpaces(LANG_SCNG__().heredoc_indentation_uses_spaces)
		ZendRestoreLexicalState(&current_state)
		LANG_SCNG__().heredoc_scan_ahead = 0
		CG__().SetIncrementLineno(0)
		CG__().SetDocComment(saved_doc_comment)
...
yy590:
	Yyleng = YYCURSOR - LANG_SCNG__().yy_text
	var heredoc_label *zend_heredoc_label = zend_ptr_stack_top(&(LANG_SCNG__().heredoc_label_stack))
	var newline int = 0
	var indentation int = 0
	var spacing int = 0
	if YYCURSOR > YYLIMIT {
	...
	for YYCURSOR < YYLIMIT {
		...
		case '\n':
			...
			if IS_LABEL_START(*YYCURSOR) && heredoc_label.GetLength() < YYLIMIT-YYCURSOR && !(memcmp(YYCURSOR, heredoc_label.GetLabel(), heredoc_label.GetLength())) {
				...
				CG__().SetIncrementLineno(1)
				if LANG_SCNG__().heredoc_scan_ahead {
					LANG_SCNG__().heredoc_indentation = indentation
					LANG_SCNG__().heredoc_indentation_uses_spaces = spacing == HEREDOC_USING_SPACES
				} else {
...
heredoc_scan_done:
	...
	if !(LANG_SCNG__().heredoc_scan_ahead) && EG__().GetException() == nil && PARSER_MODE() {
...
inline_char_handler:
	for true {
		...
		if (*YYCURSOR) == '?' {
			if CG__().GetShortTags() != 0 || (*(YYCURSOR + 1)) == '=' || !(strncasecmp((*byte)(YYCURSOR+1), "php", 3)) && (YYCURSOR+4 == YYLIMIT || YYCURSOR[4] == ' ' || YYCURSOR[4] == '\t' || YYCURSOR[4] == '\n' || YYCURSOR[4] == '\r') {
				YYCURSOR--
				break
			}
		}
	...
	if LANG_SCNG__().output_filter {
		...
		readsize = LANG_SCNG__().output_filter((**uint8)(&s), &sz, (*uint8)(Yytext), int(Yyleng))
		ZVAL_STRINGL(zendlval, s, sz)
		Efree(s)
		if readsize < Yyleng {
			Yyless(readsize)
...
yyc_ST_END_HEREDOC:
	...
	YYCURSOR++
	Yyleng = YYCURSOR - LANG_SCNG__().yy_text
	var heredoc_label *zend_heredoc_label = zend_ptr_stack_pop(&(LANG_SCNG__().heredoc_label_stack))
...
yyc_ST_NOWDOC:
	...
	YYCURSOR++
	Yyleng = YYCURSOR - LANG_SCNG__().yy_text
	var heredoc_label *ZendHeredocLabel = ZendPtrStackTop(&(LANG_SCNG__().heredoc_label_stack))
	var newline int = 0
	var indentation int = 0
	var spacing int = -1
	if YYCURSOR > YYLIMIT {
...
nowdoc_scan_done:
	Yyleng = YYCURSOR - LANG_SCNG__().yy_text
	ZVAL_STRINGL(zendlval, Yytext, Yyleng-newline)
	if EG__().GetException() == nil && spacing != -1 && PARSER_MODE() {
		var newline_at_start ZendBool = (*(Yytext - 1)) == '\n' || (*(Yytext - 1)) == '\r'
...
emit_token:
	if LANG_SCNG__().on_event {
		LANG_SCNG__().on_event(ON_TOKEN, token, start_line, LANG_SCNG__().on_event_context)
	}
	return token
return_whitespace:
	sc.HANDLE_NEWLINES(Yytext, Yyleng)
