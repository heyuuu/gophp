...
func (sc *LangScanner) parseNumStr(str string, base int) (lVal int, dVal float64, overflow bool) {
	...
	for _, c := range str {
		...
		if digit >= base {
			faults.ThrowException(faults.ZendCeParseError, "Invalid numeric literal", 0)
		}
...
func (sc *LangScanner) lexerRule21() (int, bool) {
	...
	if isHeredoc && !sc.heredocScanAhead {
		...
		var savedDocComment *types.String = CG__().doc_comment
...
heredoc_scan_done:
	...
	if !(sc.heredocScanAhead) && !(EG__().exception) && sc.isParserMode() {
		...
		var copy *types.String = types.Z_STR_P(sc.zendlval)
