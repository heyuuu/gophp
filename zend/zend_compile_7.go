...
func ZendBinaryOpProducesNumericStringError(opcode uint32, op1 *Zval, op2 *Zval) ZendBool {
	...
	if op1.IsString() && IsNumericString(b.CastStr(op1.GetStr().GetVal(), op1.GetStr().GetLen()), nil, nil, 0) == 0 {
		return 1
	}
	if op2.IsString() && IsNumericString(b.CastStr(op2.GetStr().GetVal(), op2.GetStr().GetLen()), nil, nil, 0) == 0 {
		return 1
	}
	return 0
}
