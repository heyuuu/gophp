...
func EncodingFilterScriptToInternal(to **uint8, to_length *int, from *uint8, from_length int) int {
	...
	return ZendMultibyteEncodingConverter(to, to_length, from, from_length, internal_encoding, INI_SCNG__().script_encoding)
}
func EncodingFilterScriptToIntermediate(to **uint8, to_length *int, from *uint8, from_length int) int {
	return ZendMultibyteEncodingConverter(to, to_length, from, from_length, ZendMultibyteEncodingUtf8, INI_SCNG__().script_encoding)
}
func EncodingFilterIntermediateToScript(to **uint8, to_length *int, from *uint8, from_length int) int {
	return ZendMultibyteEncodingConverter(to, to_length, from, from_length, INI_SCNG__().script_encoding, ZendMultibyteEncodingUtf8)
}
...
func ZendMultibyteFindScriptEncoding() *ZendEncoding {
	...
	if CG__().GetScriptEncodingListSize() > 1 {
		return ZendMultibyteEncodingDetector(INI_SCNG__().script_org, INI_SCNG__().script_org_size, CG__().GetScriptEncodingList(), CG__().GetScriptEncodingListSize())
	}
...
func ZendMultibyteSetFilter(onetime_encoding *ZendEncoding) int {
	...
	if internal_encoding == nil || INI_SCNG__().script_encoding == internal_encoding {
		if ZendMultibyteCheckLexerCompatibility(INI_SCNG__().script_encoding) == 0 {
	...
	if ZendMultibyteCheckLexerCompatibility(internal_encoding) != 0 {
		...
		INI_SCNG__().output_filter = nil
	} else if ZendMultibyteCheckLexerCompatibility(INI_SCNG__().script_encoding) != 0 {
		INI_SCNG__().input_filter = nil
		...
		if CG__().GetMultibyte() != 0 {
			...
			if SCNG(input_filter) {
				if size_t-1 == SCNG(input_filter)(&(SCNG(script_filtered)), &(SCNG(script_filtered_size)), SCNG(script_org), SCNG(script_org_size)) {
					ZendErrorNoreturn(E_COMPILE_ERROR, "Could not convert the script from the detected "+"encoding \"%s\" to a compatible encoding", ZendMultibyteGetEncodingName(INI_SCNG__().script_encoding))
				}
...
heredoc_scan_done:
	...
	if !(SCNG(heredoc_scan_ahead)) && EG__().GetException() == nil && PARSER_MODE() {
