...
func UserStreamCreateObject(uwrap *PhpUserStreamWrapper, context *core.PhpStreamContext, object *types.Zval) {
	...
	if uwrap.GetCe().GetConstructor() != nil {
		...
		if zend.ZendCallFunction(fci, &fcc) == types.FAILURE {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("Could not execute %s::%s()", uwrap.GetCe().Name(), uwrap.GetCe().GetConstructor().FunctionName()))
			object.SetUndef()
		} else {
		}
	}
...
func ZifStreamWrapperRestore(protocol string) bool {
	...
	if wrapper == nil {
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s:// never existed, nothing to restore", protocol))
		return false
	}

	wrapper_hash = core.PhpStreamGetUrlStreamWrappersHash()
	if wrapper_hash == global_wrapper_hash || wrapper_hash[protocol] == wrapper {
		core.PhpErrorDocref("", faults.E_NOTICE, fmt.Sprintf("%s:// was never changed, nothing to restore", protocol))
		return false
	}
	...
	if PhpRegisterUrlStreamWrapperVolatile(protocol, wrapper) == types.FAILURE {
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("Unable to restore original %s:// wrapper", protocol))
		return false
	}
	return true
}
func PhpUserstreamopWrite(stream *core.PhpStream, buf *byte, count int) ssize_t {
	...
	if call_result == types.SUCCESS && retval.IsNotUndef() {
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_WRITE+" is not implemented!", us.GetWrapper().GetClassname()))
		didwrite = -1
	}
	...
	if didwrite > 0 && didwrite > count {
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_WRITE+" wrote "+"%d"+" bytes more data than requested (%d written, %d max)", us.GetWrapper().GetClassname(), zend_long(didwrite-count), zend.ZendLong(didwrite), zend.ZendLong(count)))
		didwrite = count
	}
...
func PhpUserstreamopRead(stream *core.PhpStream, buf *byte, count int) ssize_t {
	...
	if call_result == types.FAILURE {
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_READ+" is not implemented!", us.GetWrapper().GetClassname()))
		return -1
	}
	if retval.IsType(types.IsFalse) {
		return -1
	...
	if call_result == types.SUCCESS && retval.IsNotUndef() && operators.ZvalIsTrue(&retval) {
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_EOF+" is not implemented! Assuming EOF", us.GetWrapper().GetClassname()))
		stream.SetEof(1)
	}
	// zend.ZvalPtrDtor(&retval)
	return didread
...
func PhpUserstreamopSeek(stream *core.PhpStream, offset zend.ZendOffT, whence int, newoffs *zend.ZendOffT) int {
	...
	if call_result == types.SUCCESS && retval.IsType(types.IsLong) {
		*newoffs = retval.Long()
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_TELL+" is not implemented!", us.GetWrapper().GetClassname()))
		ret = -1
	} else {
		ret = -1
	}
...
func PhpUserstreamopStat(stream *core.PhpStream, ssb *core.PhpStreamStatbuf) int {
	...
	if call_result == types.SUCCESS && retval.IsType(types.IsArray) {
		...
		if call_result == types.FAILURE {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_STAT+" is not implemented!", us.GetWrapper().GetClassname()))
		}
	}
	// zend.ZvalPtrDtor(&retval)
	// zend.ZvalPtrDtor(&func_name)
...
func PhpUserstreamopSetOption(stream *core.PhpStream, option int, value int, ptrparam any) int {
	...
	case core.PHP_STREAM_OPTION_LOCKING:
		...
		if call_result == types.SUCCESS && (retval.IsType(types.IsFalse) || retval.IsType(types.IsTrue)) {
			...
			if value == 0 {
				...
				core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_LOCK+" is not implemented!", us.GetWrapper().GetClassname()))
				ret = core.PHP_STREAM_OPTION_RETURN_ERR
			}
		}
		// zend.ZvalPtrDtor(&retval)
			...
			if new_size >= 0 && new_size <= ptrdiff_t(zend.LONG_MAX) {
				...
				if call_result == types.SUCCESS && retval.IsNotUndef() {
					if retval.IsType(types.IsFalse) || retval.IsType(types.IsTrue) {
						...
						core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_TRUNCATE+" did not return a boolean!", us.GetWrapper().GetClassname()))
					}
				} else {
					core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_TRUNCATE+" is not implemented!", us.GetWrapper().GetClassname()))
				}
				// zend.ZvalPtrDtor(&retval)
				// zend.ZvalPtrDtor(&args[0])
			} else {
	...
	case core.PHP_STREAM_OPTION_BLOCKING:
		...
		if call_result == types.FAILURE {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_SET_OPTION+" is not implemented!", us.GetWrapper().GetClassname()))
			ret = core.PHP_STREAM_OPTION_RETURN_ERR
...
func UserWrapperUnlink(wrapper *core.PhpStreamWrapper, url *byte, options int, context *core.PhpStreamContext) int {
	...
	if call_result == types.SUCCESS && (zretval.IsType(types.IsFalse) || zretval.IsType(types.IsTrue)) {
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_UNLINK+" is not implemented!", uwrap.GetClassname()))
	}
...
func UserWrapperRename(wrapper *core.PhpStreamWrapper, url_from *byte, url_to *byte, options int, context *core.PhpStreamContext) int {
	...
	if call_result == types.SUCCESS && (zretval.IsType(types.IsFalse) || zretval.IsType(types.IsTrue)) {
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_RENAME+" is not implemented!", uwrap.GetClassname()))
	}
...
func UserWrapperMkdir(wrapper *core.PhpStreamWrapper, url *byte, mode int, options int, context *core.PhpStreamContext) int {
	...
	if call_result == types.SUCCESS && (zretval.IsType(types.IsFalse) || zretval.IsType(types.IsTrue)) {
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_MKDIR+" is not implemented!", uwrap.GetClassname()))
	}
...
func UserWrapperRmdir(wrapper *core.PhpStreamWrapper, url *byte, options int, context *core.PhpStreamContext) int {
	...
	if call_result == types.SUCCESS && (zretval.IsType(types.IsFalse) || zretval.IsType(types.IsTrue)) {
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_RMDIR+" is not implemented!", uwrap.GetClassname()))
	}
...
func UserWrapperMetadata(wrapper *core.PhpStreamWrapper, url *byte, option int, value any, context *core.PhpStreamContext) int {
	...
	default:
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("Unknown option %d for "+USERSTREAM_METADATA, option))
		// zend.ZvalPtrDtor(&args[2])
		return ret
	}
	...
	if call_result == types.SUCCESS && (zretval.IsType(types.IsFalse) || zretval.IsType(types.IsTrue)) {
		...
		core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_METADATA+" is not implemented!", uwrap.GetClassname()))
	}
...
func UserWrapperStatUrl(wrapper *core.PhpStreamWrapper, url *byte, flags int, ssb *core.PhpStreamStatbuf, context *core.PhpStreamContext) int {
	...
	if call_result == types.SUCCESS && zretval.IsType(types.IsArray) {
		...
		if call_result == types.FAILURE {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_STATURL+" is not implemented!", uwrap.GetClassname()))
		}
	}
...
func PhpUserstreamopReaddir(stream *core.PhpStream, buf *byte, count int) ssize_t {
	...
	if call_result == types.SUCCESS && !retval.IsFalse() && !retval.IsTrue() {
		...
		core.PHP_STRLCPY(ent.GetDName(), retval.String(), b.SizeOf("ent -> d_name"), len(retval.String()))
...
func PhpUserstreamopCast(stream *core.PhpStream, castas int, retptr *any) int {
	...
	for {
		if call_result == types.FAILURE {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_CAST+" is not implemented!", us.GetWrapper().GetClassname()))
			break
		}
		if !operators.ZvalIsTrue(&retval) {
			break
		...
		if intstream == nil {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_CAST+" must return a stream resource", us.GetWrapper().GetClassname()))
			break
		}
		if intstream == stream {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("%s::"+USERSTREAM_CAST+" must not return itself", us.GetWrapper().GetClassname()))
			intstream = nil
			break
		}
