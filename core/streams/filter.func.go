...
func PhpStreamFilterCreate(filtername *byte, filterparams *types.Zval, persistent uint8) *core.PhpStreamFilter {
	...
	if filter == nil {
		...
		if factory == nil {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf(`unable to locate filter "%s"`, filtername))
		} else {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf(`unable to create or locate filter "%s"`, filtername))
		}
