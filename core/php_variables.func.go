		...
		for true {
			...
			if lang.PreInc(&nest_level) > PG__().max_input_nesting_level {
				...
				if !(PG__().display_errors) {
					PhpErrorDocref("", faults.E_WARNING, "Input variable nesting level exceeded %d. To increase the limit change max_input_nesting_level in php.ini.", PG__().max_input_nesting_level)
				}
				zend.FreeAlloca(varOrig, use_heap)
				return
			}
