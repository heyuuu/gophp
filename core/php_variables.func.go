		...
		for true {
			...
			if lang.PreInc(&nest_level) > PG__().max_input_nesting_level {
				...
				if !(PG__().display_errors) {
					PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("Input variable nesting level exceeded %d. To increase the limit change max_input_nesting_level in php.ini.", PG__().max_input_nesting_level))
				}
				zend.FreeAlloca(varOrig, use_heap)
				return
			}
...
func PhpDefaultTreatData(arg int, str *byte, destArray *types.Zval) {
	...
	if arg == PARSE_GET {
		c_var = SG__().RequestInfo.QueryString()
		if c_var != nil && (*c_var) {
		...
		c_var = SG__().RequestInfo.CookieData()
		if c_var != nil && (*c_var) {
			res = (*byte)(zend.Estrdup(c_var))
			free_buffer = 1
		} else {
	...
	for var_ != nil {
		...
		if lang.PreInc(&count) > PG__().max_input_vars {
			PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf("Input variables exceeded %d. To increase the limit change max_input_vars in php.ini.", PG__().max_input_vars))
			break
		}
		if val != nil {
			lang.PostInc(&(*val)) = '0'
...
func PhpBuildArgv(s string, trackVarsArray *types.Zval) {
	if SG__().RequestInfo.Argc() == 0 && trackVarsArray == nil {
		return
	}
	...
	if SG__().RequestInfo.Argc() != 0 {
		args = SG__().RequestInfo.Args()
	} else if s != "" {
		args = strings.Split(s, "+")
	}

	/* prepare argc/argv Zval */
	argcZval := types.NewZvalLong(len(args))
	argvZval := types.NewZvalArray(types.NewArrayOfString(args))
	if SG__().RequestInfo.Argc() != 0 {
...
func PhpHashEnvironment() int {
	...
	if PG__().register_argc_argv {
		PhpBuildArgv(SG__().RequestInfo.QueryString(), &PG__().http_globals[TRACK_VARS_SERVER])
	}
	return types.SUCCESS
}
