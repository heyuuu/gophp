...
func SplDualItFromObj(obj *types2.ZendObject) *SplDualItObject {
	return (*SplDualItObject)((*byte)(obj - zend_long((*byte)(&((*SplDualItObject)(nil).GetStd()))-(*byte)(nil))))
}
func Z_SPLDUAL_IT_P(zv *types2.Zval) *SplDualItObject { return SplDualItFromObj(zv.Object()) }
func SplRecursiveItFromObj(obj *types2.ZendObject) *SplRecursiveItObject {
	return (*SplRecursiveItObject)((*byte)(obj - zend_long((*byte)(&((*SplRecursiveItObject)(nil).GetStd()))-(*byte)(nil))))
}
func Z_SPLRECURSIVE_IT_P(zv *types2.Zval) *SplRecursiveItObject {
...
func SplRecursiveItValidEx(object *SplRecursiveItObject, zthis *types2.Zval) int {
	var sub_iter *zend.ZendObjectIterator
	...
	for level >= 0 {
		...
		if sub_iter.GetFuncs().GetValid()(sub_iter) == types2.SUCCESS {
...
func SplRecursiveItGetCurrentData(iter *zend.ZendObjectIterator) *types2.Zval {
...
func SplRecursiveItGetCurrentKey(iter *zend.ZendObjectIterator, key *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(iter.GetData())
	var sub_iter *zend.ZendObjectIterator = object.GetIterators()[object.GetLevel()].GetIterator()
...
func SplRecursiveItMoveForwardEx(object *SplRecursiveItObject, zthis *types2.Zval) {
	...
	next_step:
		...
		case RS_NEXT:
			...
			fallthrough
		case RS_START:
			if iterator.GetFuncs().GetValid()(iterator) == types2.FAILURE {
				break
			}
		...
		case RS_CHILD:
			...
			if child.IsUndef() || child.GetType() != types2.IS_OBJECT || !(b.Assign(&ce, types2.Z_OBJCE(child)) && zend.InstanceofFunction(ce, spl_ce_RecursiveIterator) != 0) {
				// zend.ZvalPtrDtor(&child)
			...
			types2.ZVAL_COPY_VALUE(object.GetIterators()[object.GetLevel()].GetZobject(), &child)
		...
		if object.GetLevel() > 0 {
			...
			if object.GetLevel() > 0 {
				...
				types2.ZVAL_COPY_VALUE(&garbage, object.GetIterators()[object.GetLevel()].GetZobject())
...
func SplRecursiveItRewindEx(object *SplRecursiveItObject, zthis *types2.Zval) {
	var sub_iter *zend.ZendObjectIterator
...
func SplRecursiveItGetIterator(ce *types2.ClassEntry, zobject *types2.Zval, by_ref int) *zend.ZendObjectIterator {
...
func SplRecursiveItItConstruct(executeData *zend.ZendExecuteData, return_value *types2.Zval, ce_base *types2.ClassEntry, ce_inner *types2.ClassEntry, rit_type RecursiveItItType) {
	var object *types2.Zval = zend.ZEND_THIS(executeData)
	var intern *SplRecursiveItObject
	var iterator *types2.Zval
	var ce_iterator *types2.ClassEntry
	var mode zend.ZendLong
	var flags zend.ZendLong
	var error_handling zend.ZendErrorHandling
	var caching_it types2.Zval
	var aggregate_retval types2.Zval
	zend.ZendReplaceErrorHandling(zend.EH_THROW, spl_ce_InvalidArgumentException, &error_handling)
	switch rit_type {
	case RIT_RecursiveTreeIterator:
		var caching_it_flags types2.Zval
		var user_caching_it_flags *types2.Zval = nil
		mode = RIT_SELF_FIRST
		flags = RTIT_BYPASS_KEY
		if zend.ZendParseParametersEx(zpp.FlagQuiet, executeData.NumArgs(), "o|lzl", &iterator, &flags, &user_caching_it_flags, &mode) == types2.SUCCESS {
			if zend.InstanceofFunction(types2.Z_OBJCE_P(iterator), zend.ZendCeAggregate) != 0 {
				zend.ZendCallMethodWith0Params(iterator, types2.Z_OBJCE_P(iterator), types2.Z_OBJCE_P(iterator).GetIteratorFuncsPtr().GetZfNewIterator(), "getiterator", &aggregate_retval)
				iterator = &aggregate_retval
			} else {
				// 				iterator.AddRefcount()
			}
			if user_caching_it_flags != nil {
				types2.ZVAL_COPY(&caching_it_flags, user_caching_it_flags)
			} else {
	...
	default:
		...
		flags = 0
		if zend.ZendParseParametersEx(zpp.FlagQuiet, executeData.NumArgs(), "o|ll", &iterator, &mode, &flags) == types2.SUCCESS {
			if zend.InstanceofFunction(types2.Z_OBJCE_P(iterator), zend.ZendCeAggregate) != 0 {
				zend.ZendCallMethodWith0Params(iterator, types2.Z_OBJCE_P(iterator), types2.Z_OBJCE_P(iterator).GetIteratorFuncsPtr().GetZfNewIterator(), "getiterator", &aggregate_retval)
				iterator = &aggregate_retval
			} else {
	...
	if iterator == nil || zend.InstanceofFunction(types2.Z_OBJCE_P(iterator), spl_ce_RecursiveIterator) == 0 {
		if iterator != nil {
			// zend.ZvalPtrDtor(iterator)
		}
	...
	intern.SetCe(types2.Z_OBJCE_P(object))
	...
	ce_iterator = types2.Z_OBJCE_P(iterator)
...
func zim_spl_RecursiveIteratorIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
...
func zim_spl_RecursiveIteratorIterator_rewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
...
func zim_spl_RecursiveIteratorIterator_valid(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	return_value.SetBool(SplRecursiveItValidEx(object, zend.ZEND_THIS(executeData)) == types2.SUCCESS)
	return
}
func zim_spl_RecursiveIteratorIterator_key(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var iterator *zend.ZendObjectIterator
	if !executeData.CheckNumArgsNone(false) {
		return
...
func zim_spl_RecursiveIteratorIterator_current(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var iterator *zend.ZendObjectIterator
	...
	if data != nil {
		types2.ZVAL_COPY_DEREF(return_value, data)
	}
}
func zim_spl_RecursiveIteratorIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveIteratorIterator_getDepth(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveIteratorIterator_getSubIterator(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var level zend.ZendLong = object.GetLevel()
	var value *types2.Zval
	if zend.ZendParseParameters(executeData.NumArgs(), "|l", &level) == types2.FAILURE {
		return
	}
...
func zim_spl_RecursiveIteratorIterator_getInnerIterator(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var zobject *types2.Zval
	if !executeData.CheckNumArgsNone(false) {
		return
	...
	zobject = object.GetIterators()[object.GetLevel()].GetZobject()
	types2.ZVAL_COPY_DEREF(return_value, zobject)
}
func zim_spl_RecursiveIteratorIterator_beginIteration(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func zim_spl_RecursiveIteratorIterator_endIteration(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func zim_spl_RecursiveIteratorIterator_callHasChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var ce *types2.ClassEntry
	var zobject *types2.Zval
	if !executeData.CheckNumArgsNone(false) {
...
func zim_spl_RecursiveIteratorIterator_callGetChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var ce *types2.ClassEntry
	var zobject *types2.Zval
	if !executeData.CheckNumArgsNone(false) {
...
func zim_spl_RecursiveIteratorIterator_beginChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func zim_spl_RecursiveIteratorIterator_endChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func zim_spl_RecursiveIteratorIterator_nextElement(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func zim_spl_RecursiveIteratorIterator_setMaxDepth(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var max_depth zend.ZendLong = -1
	if zend.ZendParseParameters(executeData.NumArgs(), "|l", &max_depth) == types2.FAILURE {
		return
	}
	if max_depth < -1 {
...
func zim_spl_RecursiveIteratorIterator_getMaxDepth(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func SplRecursiveItGetMethod(zobject **types2.ZendObject, method *types2.String, key *types2.Zval) types2.IFunction {
	var function_handler types2.IFunction
	var object *SplRecursiveItObject = SplRecursiveItFromObj(*zobject)
	var level zend.ZendLong = object.GetLevel()
	var zobj *types2.Zval
	if object.GetIterators() == nil {
	...
	if function_handler == nil {
		if b.Assign(&function_handler, types2.Z_OBJCE_P(zobj).FunctionTable().Get(method.GetStr())) == nil {
			*zobject = zobj.Object()
...
func spl_RecursiveIteratorIterator_dtor(_object *types2.ZendObject) {
...
func spl_RecursiveIteratorIterator_free_storage(_object *types2.ZendObject) {
	var object *SplRecursiveItObject = SplRecursiveItFromObj(_object)
	if object.GetIterators() != nil {
		zend.Efree(object.GetIterators())
		object.SetIterators(nil)
...
func spl_RecursiveIteratorIterator_new_ex(class_type *types2.ClassEntry, init_prefix int) *types2.ZendObject {
	var intern *SplRecursiveItObject
...
func spl_RecursiveIteratorIterator_new(class_type *types2.ClassEntry) *types2.ZendObject {
	return spl_RecursiveIteratorIterator_new_ex(class_type, 0)
}
func spl_RecursiveTreeIterator_new(class_type *types2.ClassEntry) *types2.ZendObject {
	return spl_RecursiveIteratorIterator_new_ex(class_type, 1)
}
func SplRecursiveTreeIteratorGetPrefix(object *SplRecursiveItObject, return_value *types2.Zval) {
	var str zend.SmartStr = zend.MakeSmartStr(0)
	var has_next types2.Zval
	var level int
...
func SplRecursiveTreeIteratorGetEntry(object *SplRecursiveItObject, return_value *types2.Zval) {
	...
	if data != nil {
		...
		if data.IsType(types2.IS_ARRAY) {
			return_value.SetStringVal(types2.STR_ARRAY_CAPITALIZED)
		} else {
			types2.ZVAL_COPY(return_value, data)
			zend.ConvertToString(return_value)
		}
	}
}
func SplRecursiveTreeIteratorGetPostfix(object *SplRecursiveItObject, return_value *types2.Zval) {
	return_value.SetString(object.GetPostfix()[0].GetS())
	// 	return_value.AddRefcount()
}
func zim_spl_RecursiveTreeIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
...
func zim_spl_RecursiveTreeIterator_setPrefixPart(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var part zend.ZendLong
	var prefix *byte
	var prefix_len int
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if zend.ZendParseParameters(executeData.NumArgs(), "ls", &part, &prefix, &prefix_len) == types2.FAILURE {
		return
	}
	if 0 > part || part > 5 {
		faults.ThrowExceptionEx(spl_ce_OutOfRangeException, 0, "Use RecursiveTreeIterator::PREFIX_* constant")
		return
	}
	object.GetPrefix()[part].Free()
	object.GetPrefix()[part].AppendString(b.CastStr(prefix, prefix_len))
}
func zim_spl_RecursiveTreeIterator_getPrefix(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveTreeIterator_setPostfix(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var postfix *byte
	var postfix_len int
	if zend.ZendParseParameters(executeData.NumArgs(), "s", &postfix, &postfix_len) == types2.FAILURE {
		return
	}
	object.GetPostfix()[0].Free()
	object.GetPostfix()[0].AppendString(b.CastStr(postfix, postfix_len))
}
func zim_spl_RecursiveTreeIterator_getEntry(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveTreeIterator_getPostfix(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveTreeIterator_current(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var prefix types2.Zval
	var entry types2.Zval
	var postfix types2.Zval
	var ptr *byte
	...
	if entry.GetType() != types2.IS_STRING {
	...
	str = types2.ZendStringAlloc(prefix.String().GetLen()+entry.String().GetLen()+postfix.String().GetLen(), 0)
	ptr = str.GetVal()
...
func zim_spl_RecursiveTreeIterator_key(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var object *SplRecursiveItObject = Z_SPLRECURSIVE_IT_P(zend.ZEND_THIS(executeData))
	var iterator *zend.ZendObjectIterator
	var prefix types2.Zval
	var key types2.Zval
	var postfix types2.Zval
	var key_copy types2.Zval
	var ptr *byte
	var str *types2.String
	if !executeData.CheckNumArgsNone(false) {
		return
	...
	if key.GetType() != types2.IS_STRING {
	...
	SplRecursiveTreeIteratorGetPrefix(object, &prefix)
	SplRecursiveTreeIteratorGetPostfix(object, &postfix)
	str = types2.ZendStringAlloc(prefix.String().GetLen()+key.String().GetLen()+postfix.String().GetLen(), 0)
	ptr = str.GetVal()
	memcpy(ptr, prefix.String().GetVal(), prefix.String().GetLen())
	ptr += prefix.String().GetLen()
...
func SplDualItGetMethod(object **types2.ZendObject, method *types2.String, key *types2.Zval) types2.IFunction {
	var function_handler types2.IFunction
	...
	if function_handler == nil && intern.GetCe() != nil {
		if b.Assign(&function_handler, intern.GetCe().FunctionTable().Get(method.GetStr())) == nil {
			if types2.Z_OBJ_HT(intern.GetZobject()).GetGetMethod() != nil {
...
func SplDualItConstruct(executeData *zend.ZendExecuteData, return_value *types2.Zval, ce_base *types2.ClassEntry, ce_inner *types2.ClassEntry, dit_type DualItType) *SplDualItObject {
	var zobject *types2.Zval
	var retval types2.Zval
	var intern *SplDualItObject
	...
	case DIT_LimitIterator:
		intern.SetOffset(0)
		intern.SetCount(-1)
		if zend.ZendParseParametersThrow(executeData.NumArgs(), "O|ll", &zobject, ce_inner, intern.GetOffset(), intern.GetCount()) == types2.FAILURE {
			return nil
		}
	...
	case DIT_RecursiveCachingIterator:
		...
		if zend.ZendParseParametersThrow(executeData.NumArgs(), "O|l", &zobject, ce_inner, &flags) == types2.FAILURE {
			return nil
		}
		if SplCitCheckFlags(flags) != types2.SUCCESS {
	...
	case DIT_IteratorIterator:
		var ce_cast *types2.ClassEntry
		var class_name *types2.String
		if zend.ZendParseParametersThrow(executeData.NumArgs(), "O|S", &zobject, ce_inner, &class_name) == types2.FAILURE {
			return nil
		}
		ce = types2.Z_OBJCE_P(zobject)
		if zend.InstanceofFunction(ce, zend.ZendCeIterator) == 0 {
			...
			if zend.InstanceofFunction(ce, zend.ZendCeAggregate) != 0 {
				...
				if retval.GetType() != types2.IS_OBJECT || zend.InstanceofFunction(types2.Z_OBJCE(retval), zend.ZendCeTraversable) == 0 {
	...
	case DIT_RecursiveRegexIterator:
		...
		if zend.ZendParseParametersThrow(executeData.NumArgs(), "OS|lll", &zobject, ce_inner, &regex, &mode, intern.GetURegexFlags(), intern.GetPregFlags()) == types2.FAILURE {
			return nil
		}
	...
	case DIT_RecursiveCallbackFilterIterator:
		...
		if zend.ZendParseParametersThrow(executeData.NumArgs(), "Of", &zobject, ce_inner, cfi.GetFci(), cfi.GetFcc()) == types2.FAILURE {
			zend.Efree(cfi)
			return nil
		}
	...
	default:
		if zend.ZendParseParametersThrow(executeData.NumArgs(), "O", &zobject, ce_inner) == types2.FAILURE {
			return nil
		}
	}
	if inc_refcount != 0 {
	...
	if dit_type == DIT_IteratorIterator {
		...
		intern.SetCe(types2.Z_OBJCE_P(zobject))
	}
	intern.SetObject(zobject.Object())
	intern.SetInnerIterator(intern.GetCe().GetGetIterator()(intern.GetCe(), zobject, 0))
	return intern
}
func zim_spl_FilterIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_FilterIterator, zend.ZendCeIterator, DIT_FilterIterator)
}
func zim_spl_CallbackFilterIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_CallbackFilterIterator, zend.ZendCeIterator, DIT_CallbackFilterIterator)
}
func zim_spl_dual_it_getInnerIterator(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	if !(intern.GetZobject().IsUndef()) {
		var value *types2.Zval = intern.GetZobject()
		types2.ZVAL_COPY_DEREF(return_value, value)
	} else {
		return_value.SetNull()
		return
...
func SplDualItFetch(intern *SplDualItObject, check_more int) int {
	var data *types2.Zval
	SplDualItFree(intern)
	if check_more == 0 || SplDualItValid(intern) == types2.SUCCESS {
		data = intern.GetInnerIterator().GetFuncs().GetGetCurrentData()(intern.GetInnerIterator())
		if data != nil {
			types2.ZVAL_COPY(intern.GetData(), data)
		}
...
func ZimSplDualItRewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func ZimSplDualItValid(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func ZimSplDualItKey(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	intern = it
	if intern.GetKey().IsNotUndef() {
		var value *types2.Zval = intern.GetKey()
		types2.ZVAL_COPY_DEREF(return_value, value)
	} else {
		return_value.SetNull()
		return
...
func ZimSplDualItCurrent(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	intern = it
	if intern.GetData().IsNotUndef() {
		var value *types2.Zval = intern.GetData()
		types2.ZVAL_COPY_DEREF(return_value, value)
	} else {
		return_value.SetNull()
		return
...
func ZimSplDualItNext(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func SplFilterItFetch(zthis *types2.Zval, intern *SplDualItObject) {
	var retval types2.Zval
	for SplDualItFetch(intern, 1) == types2.SUCCESS {
...
func SplFilterItRewind(zthis *types2.Zval, intern *SplDualItObject) {
	SplDualItRewind(intern)
	SplFilterItFetch(zthis, intern)
}
func SplFilterItNext(zthis *types2.Zval, intern *SplDualItObject) {
	SplDualItNext(intern, 1)
	SplFilterItFetch(zthis, intern)
}
func zim_spl_FilterIterator_rewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_FilterIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveCallbackFilterIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
...
func zim_spl_RecursiveFilterIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
...
func zim_spl_RecursiveFilterIterator_hasChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var retval types2.Zval
	if !executeData.CheckNumArgsNone(false) {
		return
...
func zim_spl_RecursiveFilterIterator_getChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var retval types2.Zval
	if !executeData.CheckNumArgsNone(false) {
		return
	...
	if zend.EG__().GetException() == nil && retval.IsNotUndef() {
		SplInstantiateArgEx1(types2.Z_OBJCE_P(zend.ZEND_THIS(executeData)), return_value, &retval)
	}
	// zend.ZvalPtrDtor(&retval)
}
func zim_spl_RecursiveCallbackFilterIterator_getChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var retval types2.Zval
	if !executeData.CheckNumArgsNone(false) {
		return
	...
	zend.ZendCallMethodWith0Params(intern.GetZobject(), intern.GetCe(), nil, "getchildren", &retval)
	if zend.EG__().GetException() == nil && retval.IsNotUndef() {
		SplInstantiateArgEx2(types2.Z_OBJCE_P(zend.ZEND_THIS(executeData)), return_value, &retval, intern.GetCbfilter().GetFci().GetFunctionName())
	}
	// zend.ZvalPtrDtor(&retval)
}
func zim_spl_ParentIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_ParentIterator, spl_ce_RecursiveIterator, DIT_ParentIterator)
}
func zim_spl_RegexIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_RegexIterator, zend.ZendCeIterator, DIT_RegexIterator)
}
func zim_spl_CallbackFilterIterator_accept(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	var fci *types2.ZendFcallInfo = intern.GetCbfilter().GetFci()
	var fcc *types2.ZendFcallInfoCache = intern.GetCbfilter().GetFcc()
	...
	types2.ZVAL_COPY_VALUE(&params[0], intern.GetData())
	types2.ZVAL_COPY_VALUE(&params[1], intern.GetKey())
	types2.ZVAL_COPY_VALUE(&params[2], intern.GetZobject())
	fci.SetRetval(return_value)
	fci.SetParamCount(3)
	fci.SetParams(params)
	fci.SetNoSeparation(0)
	if zend.ZendCallFunction(fci, fcc) != types2.SUCCESS || return_value.IsUndef() {
		return_value.SetFalse()
		return
	}
	if zend.EG__().GetException() != nil {
	...
	types2.ZVAL_COPY_VALUE(intern.GetData(), &params[0])
	types2.ZVAL_COPY_VALUE(intern.GetKey(), &params[1])
}
func zim_spl_RegexIterator_accept(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var result *types2.String
	var subject *types2.String
	var count int = 0
	var zcount types2.Zval
	var rv types2.Zval
	...
	if intern.IsUseKey() {
		...
		if intern.GetData().IsType(types2.IS_ARRAY) {
			return_value.SetFalse()
			return
		}
	...
	case REGIT_MODE_REPLACE:
		var replacement *types2.Zval = zend.ZendReadProperty(intern.GetStd().GetCe(), zend.ZEND_THIS(executeData), "replacement", 1, &rv)
		var replacement_str *types2.String = zend.ZvalTryGetString(replacement)
	...
	if intern.IsInverted() {
		return_value.SetBool(return_value.GetType() != types2.IS_TRUE)
	}
	// types.ZendStringReleaseEx(subject, 0)
}
func zim_spl_RegexIterator_getRegex(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RegexIterator_getMode(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RegexIterator_setMode(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var mode zend.ZendLong
	if zend.ZendParseParameters(executeData.NumArgs(), "l", &mode) == types2.FAILURE {
		return
	}
	if mode < 0 || mode >= REGIT_MODE_MAX {
		faults.ThrowExceptionEx(spl_ce_InvalidArgumentException, 0, "Illegal mode "+zend.ZEND_LONG_FMT, mode)
...
func zim_spl_RegexIterator_getFlags(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RegexIterator_setFlags(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var flags zend.ZendLong
	if zend.ZendParseParameters(executeData.NumArgs(), "l", &flags) == types2.FAILURE {
		return
	}
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
...
func zim_spl_RegexIterator_getPregFlags(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RegexIterator_setPregFlags(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var preg_flags zend.ZendLong
	if zend.ZendParseParameters(executeData.NumArgs(), "l", &preg_flags) == types2.FAILURE {
		return
	}
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
...
func zim_spl_RecursiveRegexIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
...
func zim_spl_RecursiveRegexIterator_getChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var retval types2.Zval
	if !executeData.CheckNumArgsNone(false) {
		return
	...
	if zend.EG__().GetException() == nil {
		...
		types2.ZVAL_COPY(&args[0], &retval)
		...
		SplInstantiateArgN(types2.Z_OBJCE_P(zend.ZEND_THIS(executeData)), return_value, 5, args)
		// zend.ZvalPtrDtor(&args[0])
		// zend.ZvalPtrDtor(&args[1])
	}
	// zend.ZvalPtrDtor(&retval)
}
func zim_spl_RecursiveRegexIterator_accept(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	if intern.GetData().IsUndef() {
		return_value.SetFalse()
		return
	} else if intern.GetData().IsType(types2.IS_ARRAY) {
...
func SplDualItDtor(_object *types2.ZendObject) {
...
func SplDualItFreeStorage(_object *types2.ZendObject) {
	var object *SplDualItObject = SplDualItFromObj(_object)
...
func SplDualItNew(class_type *types2.ClassEntry) *types2.ZendObject {
	var intern *SplDualItObject
...
func SplLimitItSeek(intern *SplDualItObject, pos zend.ZendLong) {
	...
	if pos != intern.GetPos() && zend.InstanceofFunction(intern.GetCe(), spl_ce_SeekableIterator) != 0 {
		...
		if zend.EG__().GetException() == nil {
			intern.SetPos(pos)
			if SplLimitItValid(intern) == types2.SUCCESS {
		...
		for pos > intern.GetPos() && SplDualItValid(intern) == types2.SUCCESS {
			SplDualItNext(intern, 1)
		}
		if SplDualItValid(intern) == types2.SUCCESS {
			SplDualItFetch(intern, 1)
		}
	}
}
func zim_spl_LimitIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_LimitIterator, zend.ZendCeIterator, DIT_LimitIterator)
}
func zim_spl_LimitIterator_rewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
		faults.ThrowExceptionEx(spl_ce_LogicException, 0, "The object is in an invalid state as the parent constructor was not called")
...
func zim_spl_LimitIterator_valid(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
		faults.ThrowExceptionEx(spl_ce_LogicException, 0, "The object is in an invalid state as the parent constructor was not called")
...
func zim_spl_LimitIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
		faults.ThrowExceptionEx(spl_ce_LogicException, 0, "The object is in an invalid state as the parent constructor was not called")
...
func zim_spl_LimitIterator_seek(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var pos zend.ZendLong
	if zend.ZendParseParameters(executeData.NumArgs(), "l", &pos) == types2.FAILURE {
		return
	}
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
...
func zim_spl_LimitIterator_getPosition(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
		faults.ThrowExceptionEx(spl_ce_LogicException, 0, "The object is in an invalid state as the parent constructor was not called")
...
func SplCachingItNext(intern *SplDualItObject) {
	if SplDualItFetch(intern, 1) == types2.SUCCESS {
		intern.SetIsValid(true)

		/* Full cache ? */

		if intern.IsFullCache() {
			var key *types2.Zval = intern.GetKey()
			var data *types2.Zval = intern.GetData()
			data = types2.ZVAL_DEREF(data)
			//data.TryAddRefcount()
		...
		if intern.GetDitType() == DIT_RecursiveCachingIterator {
			var retval types2.Zval
			var zchildren types2.Zval
			var zflags types2.Zval
			zend.ZendCallMethodWith0Params(intern.GetZobject(), intern.GetCe(), nil, "haschildren", &retval)
			if zend.EG__().GetException() != nil {
				// zend.ZvalPtrDtor(&retval)
		...
		if intern.HasUCachingFlags(CIT_TOSTRING_USE_INNER | CIT_CALL_TOSTRING) {
			var use_copy int
			var expr_copy types2.Zval
			if intern.IsTostringUseInner() {
				types2.ZVAL_COPY_VALUE(intern.GetZstr(), intern.GetZobject())
			} else {
				types2.ZVAL_COPY_VALUE(intern.GetZstr(), intern.GetData())
			}
			use_copy = zend.ZendMakePrintableZval(intern.GetZstr(), &expr_copy)
			if use_copy != 0 {
				types2.ZVAL_COPY_VALUE(intern.GetZstr(), &expr_copy)
			} else {
...
func zim_spl_CachingIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_CachingIterator, zend.ZendCeIterator, DIT_CachingIterator)
}
func zim_spl_CachingIterator_rewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_CachingIterator_valid(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	intern = it
	return_value.SetBool(SplCachingItValid(intern) == types2.SUCCESS)
	return
}
func zim_spl_CachingIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_CachingIterator_hasNext(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	intern = it
	return_value.SetBool(SplCachingItHasNext(intern) == types2.SUCCESS)
	return
}
func zim_spl_CachingIterator___toString(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
		faults.ThrowExceptionEx(spl_ce_LogicException, 0, "The object is in an invalid state as the parent constructor was not called")
	...
	if !intern.HasUCachingFlags(CIT_CALL_TOSTRING | CIT_TOSTRING_USE_KEY | CIT_TOSTRING_USE_CURRENT | CIT_TOSTRING_USE_INNER) {
		faults.ThrowExceptionEx(spl_ce_BadMethodCallException, 0, "%s does not fetch string value (see CachingIterator::__construct)", types2.Z_OBJCE_P(zend.ZEND_THIS(executeData)).GetName().GetVal())
		return
	}
	if intern.IsTostringUseKey() {
		types2.ZVAL_COPY(return_value, intern.GetKey())
		zend.ConvertToString(return_value)
		return
	} else if intern.IsTostringUseCurrent() {
		types2.ZVAL_COPY(return_value, intern.GetData())
		zend.ConvertToString(return_value)
		return
	}
	if intern.GetZstr().IsType(types2.IS_STRING) {
...
func zim_spl_CachingIterator_offsetSet(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	...
	intern = it
	if !intern.IsFullCache() {
		faults.ThrowExceptionEx(spl_ce_BadMethodCallException, 0, "%s does not use a full cache (see CachingIterator::__construct)", types2.Z_OBJCE_P(zend.ZEND_THIS(executeData)).GetName().GetVal())
		return
	}
	if zend.ZendParseParameters(executeData.NumArgs(), "Sz", &key, &value) == types2.FAILURE {
		return
	}
	//value.TryAddRefcount()
	intern.GetZcache().Array().SymtableUpdate(key.GetStr(), value)
}
func zim_spl_CachingIterator_offsetGet(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var key *types2.String
	var value *types2.Zval
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	...
	if zend.ZendParseParameters(executeData.NumArgs(), "S", &key) == types2.FAILURE {
		return
	}
...
func zim_spl_CachingIterator_offsetUnset(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var key *types2.String
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
...
func zim_spl_CachingIterator_offsetExists(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var key *types2.String
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
...
func zim_spl_CachingIterator_getCache(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	types2.ZVAL_COPY(return_value, intern.GetZcache())
}
func zim_spl_CachingIterator_getFlags(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_CachingIterator_setFlags(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var flags zend.ZendLong
	var it *SplDualItObject = Z_SPLDUAL_IT_P(zend.ZEND_THIS(executeData))
	if it.GetDitType() == DIT_Unknown {
...
func zim_spl_CachingIterator_count(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveCachingIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
...
func zim_spl_RecursiveCachingIterator_hasChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_RecursiveCachingIterator_getChildren(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	intern = it
	if intern.GetZchildren().IsNotUndef() {
		var value *types2.Zval = intern.GetZchildren()
		types2.ZVAL_COPY_DEREF(return_value, value)
	} else {
		return_value.SetNull()
		return
...
func zim_spl_IteratorIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_IteratorIterator, zend.ZendCeTraversable, DIT_IteratorIterator)
}
func zim_spl_NoRewindIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_NoRewindIterator, zend.ZendCeIterator, DIT_NoRewindIterator)
}
func zim_spl_NoRewindIterator_rewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func zim_spl_NoRewindIterator_valid(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	...
	intern = it
	return_value.SetBool(intern.GetInnerIterator().GetFuncs().GetValid()(intern.GetInnerIterator()) == types2.SUCCESS)
	return
}
func zim_spl_NoRewindIterator_key(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_NoRewindIterator_current(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var data *types2.Zval
	if !executeData.CheckNumArgsNone(false) {
		return
...
func zim_spl_NoRewindIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_InfiniteIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_InfiniteIterator, zend.ZendCeIterator, DIT_InfiniteIterator)
}
func zim_spl_InfiniteIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_EmptyIterator_rewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func zim_spl_EmptyIterator_valid(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	return_value.SetFalse()
...
func zim_spl_EmptyIterator_key(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	faults.ThrowException(spl_ce_BadMethodCallException, "Accessing the key of an EmptyIterator", 0)
}
func zim_spl_EmptyIterator_current(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
	faults.ThrowException(spl_ce_BadMethodCallException, "Accessing the value of an EmptyIterator", 0)
}
func zim_spl_EmptyIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	if !executeData.CheckNumArgsNone(false) {
		return
	}
}
func SplAppendItNextIterator(intern *SplDualItObject) int {
	...
	if intern.GetUAppendIterator().GetFuncs().GetValid()(intern.GetUAppendIterator()) == types2.SUCCESS {
		var it *types2.Zval
		it = intern.GetUAppendIterator().GetFuncs().GetGetCurrentData()(intern.GetUAppendIterator())
		types2.ZVAL_COPY(intern.GetZobject(), it)
		intern.SetCe(types2.Z_OBJCE_P(it))
...
func SplAppendItFetch(intern *SplDualItObject) {
	for SplDualItValid(intern) != types2.SUCCESS {
		intern.GetUAppendIterator().GetFuncs().GetMoveForward()(intern.GetUAppendIterator())
		if SplAppendItNextIterator(intern) != types2.SUCCESS {
			return
		}
	}
	SplDualItFetch(intern, 0)
...
func zim_spl_AppendIterator___construct(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	SplDualItConstruct(executeData, return_value, spl_ce_AppendIterator, zend.ZendCeIterator, DIT_AppendIterator)
}
func zim_spl_AppendIterator_append(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var it *types2.Zval
	...
	intern = it__1
	if zend.ZendParseParametersEx(zpp.FlagQuiet, executeData.NumArgs(), "O", &it, zend.ZendCeIterator) == types2.FAILURE {
		return
	}
	if intern.GetUAppendIterator().GetFuncs().GetValid()(intern.GetUAppendIterator()) == types2.SUCCESS && SplDualItValid(intern) != types2.SUCCESS {
		SplArrayIteratorAppend(intern.GetZarrayit(), it)
		intern.GetUAppendIterator().GetFuncs().GetMoveForward()(intern.GetUAppendIterator())
	} else {
		SplArrayIteratorAppend(intern.GetZarrayit(), it)
	}
	if intern.GetInnerIterator() == nil || SplDualItValid(intern) != types2.SUCCESS {
		if intern.GetUAppendIterator().GetFuncs().GetValid()(intern.GetUAppendIterator()) != types2.SUCCESS {
...
func zim_spl_AppendIterator_current(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_AppendIterator_rewind(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_AppendIterator_valid(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_AppendIterator_next(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_AppendIterator_getIteratorIndex(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	if !executeData.CheckNumArgsNone(false) {
		return
	}
...
func zim_spl_AppendIterator_getArrayIterator(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var intern *SplDualItObject
	var value *types2.Zval
	if !executeData.CheckNumArgsNone(false) {
		return
...
func SplIteratorApply(obj *types2.Zval, apply_func SplIteratorApplyFuncT, puser any) int {
	var iter *zend.ZendObjectIterator
	var ce *types2.ClassEntry = types2.Z_OBJCE_P(obj)
	...
	for iter.GetFuncs().GetValid()(iter) == types2.SUCCESS {
		...
		if apply_func(iter, puser) == types2.ArrayApplyStop || zend.EG__().GetException() != nil {
			goto done
		}
		iter.GetIndex()++
...
func SplIteratorToArrayApply(iter *zend.ZendObjectIterator, puser any) int {
	var data *types2.Zval
	var return_value *types2.Zval = (*types2.Zval)(puser)
	data = iter.GetFuncs().GetGetCurrentData()(iter)
	if zend.EG__().GetException() != nil {
		return types2.ArrayApplyStop
	}
	if data == nil {
		return types2.ArrayApplyStop
	}
...
func ZifIteratorToArray(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var obj *types2.Zval
	var use_keys types2.ZendBool = 1
	if zend.ZendParseParameters(executeData.NumArgs(), "O|b", &obj, zend.ZendCeTraversable, &use_keys) == types2.FAILURE {
		return_value.SetFalse()
		return
	}
...
func ZifIteratorCount(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var obj *types2.Zval
	var count zend.ZendLong = 0
	if zend.ZendParseParameters(executeData.NumArgs(), "O", &obj, zend.ZendCeTraversable) == types2.FAILURE {
		return_value.SetFalse()
		return
	}
	if SplIteratorApply(obj, SplIteratorCountApply, any(&count)) == types2.FAILURE {
		return
	}
...
func SplIteratorFuncApply(iter *zend.ZendObjectIterator, puser any) int {
	...
	if zend.ZendIsTrue(&retval) != 0 {
		result = types2.ArrayApplyKeep
	} else {
		result = types2.ArrayApplyStop
	}
	// zend.ZvalPtrDtor(&retval)
	return result
}
func ZifIteratorApply(executeData *zend.ZendExecuteData, return_value *types2.Zval) {
	var apply_info SplIteratorApplyInfo
	apply_info.SetArgs(nil)
	if zend.ZendParseParameters(executeData.NumArgs(), "Of|a!", apply_info.GetObj(), zend.ZendCeTraversable, apply_info.GetFci(), apply_info.GetFcc(), apply_info.GetArgs()) == types2.FAILURE {
		return
	}
	apply_info.SetCount(0)
	zend.ZendFcallInfoArgs(apply_info.GetFci(), apply_info.GetArgs())
	if SplIteratorApply(apply_info.GetObj(), SplIteratorFuncApply, any(&apply_info)) == types2.FAILURE {
