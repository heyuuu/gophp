...
func ZmStartupUserFilters(module_number int) int {
	...
	zend.ZendDeclarePropertyString(phpUserFilter, "filtername", "", types.AccPublic)
	zend.ZendDeclarePropertyString(phpUserFilter, "params", "", types.AccPublic)
...
func UserFilterFactoryCreate(filtername *byte, filterparams *types.Zval, persistent uint8) *core.PhpStreamFilter {
	...
	if nil == fdat {
		...
		if fdat == nil {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf(`Err, filter "%s" is not in the user-filter map, but somehow the user-filter-factory was invoked for it!?`, filtername))
			return nil
		}
	}
	...
	if fdat.GetCe() == nil {
		if nil == lang.Assign(&(fdat.GetCe()), zend.ZendLookupClass(fdat.GetClassname())) {
			core.PhpErrorDocref("", faults.E_WARNING, fmt.Sprintf(`user-filter "%s" requires class "%s", but that class is not defined`, filtername, fdat.GetClassname()))
			return nil
		}
	}
...
func PhpStreamBucketAttach(append int, executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	if nil != lang.Assign(&pzdata, types.ZendHashStrFindDeref(types.Z_OBJPROP_P(zobject), "data")) && pzdata.IsString() {
		...
		memcpy(bucket.GetBuf(), pzdata.StringEx().GetVal(), bucket.GetBuflen())
	}
	if append != 0 {
