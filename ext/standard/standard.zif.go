...
var DefZifHrtime = def.DefFunc("hrtime", 0, 0, []def.ArgInfo{}, func(executeData zpp.Ex, returnValue zpp.Ret) {
	if !zpp.CheckNumArgsNoneError(executeData) {
		return
	}
	ret := ZifHrtime()
...
var DefZifIptcembed = def.DefFunc("iptcembed", 2, 3, []def.ArgInfo{{Name: "iptc_data"}, {Name: "filename"}, {Name: "spool"}}, func(executeData zpp.Ex, returnValue zpp.Ret) {
	fp := zpp.FastParseStart(executeData, 2, 3, 0)
	iptc_data := fp.ParseStringVal()
	...
	ret := ZifIptcembed(iptc_data, filename, nil, spool)
	returnValue.SetBy(ret)
})
...
var DefZifIptcparse = def.DefFunc("iptcparse", 1, 1, []def.ArgInfo{{Name: "iptcdata"}}, func(executeData zpp.Ex, returnValue zpp.Ret) {
	fp := zpp.FastParseStart(executeData, 1, 1, 0)
	iptcdata := fp.ParseStringVal()
	if fp.HasError() {
		return
	}
	ret, ok := ZifIptcparse(iptcdata)
	if ok {
		returnValue.SetArray(ret)
	} else {
		returnValue.SetFalse()
