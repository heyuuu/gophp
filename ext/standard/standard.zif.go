...
// generate by ZifFopen
var DefZifFopen = def.DefFunc("fopen", 2, 4, []def.ArgInfo{{Name: "filename"}, {Name: "mode"}, {Name: "use_include_path"}, {Name: "context"}}, func(executeData *php.ExecuteData, returnValue zpp.Ret) {
	fp := php.NewParamParser(executeData, 2, 4, 0)
	fp.CheckNumArgs()
	filename := fp.ParsePath()
	mode := fp.ParseString()
	fp.StartOptional()
	use_include_path := fp.ParseBool()
	context_ := fp.ParseResourceNullable()
	if fp.HasError() {
		return
	}
	ret, ok := ZifFopen(executeData.Ctx(), filename, mode, nil, use_include_path, context_)
	if ok {
		returnValue.SetResource(ret)
	} else {
		returnValue.SetFalse()
	}
})
