...
func PhpNatsort(executeData *zend.ZendExecuteData, return_value *types.Zval, fold_case int) {
	...
	if fold_case != 0 {
		if !array.GetArr().SortCompatible(PhpArrayNaturalCaseCompare, 0) {
			return
		}
	} else {
		if !array.GetArr().SortCompatible(PhpArrayNaturalCompare, 0) {
			return
		}
	}
...
func ZifMin(executeData zpp.DefEx, return_value zpp.DefReturn, args []*types.Zval) {
	var argc int
...
func ZifMax(executeData zpp.DefEx, return_value zpp.DefReturn, args []*types.Zval) {
	var args *types.Zval = nil
	var argc int
	for {
		var _flags = 0
	...
	for {
		...
		if zv.IsIndirect() {
			...
			if zv.GetType() != types.IS_REFERENCE && array.IsType(types.IS_OBJECT) {
				var prop_info = zend.ZendGetTypedPropertyInfoForSlot(array.GetObj(), zv)
				if prop_info != nil {
					zv.SetNewRef(zv)
...
func ZifArrayWalk(executeData zpp.DefEx, return_value zpp.DefReturn, input zpp.DefRef, funcname *types.Zval, _ zpp.DefOpt, userdata *types.Zval) {
	var array *types.Zval
...
func ZifArrayWalkRecursive(executeData zpp.DefEx, return_value zpp.DefReturn, input zpp.DefRef, funcname *types.Zval, _ zpp.DefOpt, userdata *types.Zval) {
	var array *types.Zval
	var userdata *types.Zval = nil
	var orig_array_walk_fci types.ZendFcallInfo
	var orig_array_walk_fci_cache types.ZendFcallInfoCache
...
func ZifInArray(executeData zpp.DefEx, return_value zpp.DefReturn, needle *types.Zval, haystack *types.Zval, _ zpp.DefOpt, strict *types.Zval) {
	PhpSearchArray(executeData, return_value, 0)
}
func ZifArraySearch(executeData zpp.DefEx, return_value zpp.DefReturn, needle *types.Zval, haystack *types.Zval, _ zpp.DefOpt, strict *types.Zval) {
...
func PhpCompactVar(eg_active_symbol_table *types.Array, return_value *types.Zval, entry *types.Zval) {
	...
	if entry.IsType(types.IS_STRING) {
		if b.Assign(&value_ptr, types.ZendHashFindInd(eg_active_symbol_table, entry.GetStr().GetStr())) != nil {
			...
			core.PhpErrorDocref(nil, faults.E_NOTICE, "Undefined variable: %s", entry.GetStr().GetVal())
		}
...
func ZifCompact(executeData zpp.DefEx, return_value zpp.DefReturn, varNames []*types.Zval) {
	var args *types.Zval = nil
	var num_args uint32
	var i uint32
	var symbol_table *types.Array
	...
	if num_args != 0 && args[0].IsType(types.IS_ARRAY) {
		zend.ArrayInitSize(return_value, types.Z_ARRVAL(args[0]).Len())
	} else {
...
func ZifArrayFillKeys(executeData zpp.DefEx, return_value zpp.DefReturn, keys *types.Zval, val *types.Zval) {
	...
	zend.ArrayInitSize(return_value, types.Z_ARRVAL_P(keys).Len())
	var __ht = keys.GetArr()
	for _, _p := range __ht.ForeachData() {
		var _z = _p.GetVal()
...
func ZifRange(executeData zpp.DefEx, return_value zpp.DefReturn, low *types.Zval, high *types.Zval, _ zpp.DefOpt, step *types.Zval) {
	var zlow *types.Zval
...
func PhpArrayDataShuffle(array *types.Zval) {
	...
	var n_left uint32
	n_elems = types.Z_ARRVAL_P(array).Len()
	if n_elems < 1 {
		return
	}
	hash = array.GetArr()
...
func PhpSplice(in_hash *types.Array, offset zend.ZendLong, length zend.ZendLong, replace *types.Array, removed *types.Array) {
	...
	&out_hash = types.MakeArrayEx(b.Cond(length > 0, num_in-length, 0)+b.CondF1(replace != nil, func() __auto__ { return replace.Len() }, 0), zend.ZVAL_PTR_DTOR, 0)
...
func ZifArrayPush(executeData zpp.DefEx, return_value zpp.DefReturn, stack zpp.DefRef, _ zpp.DefOpt, vars []*types.Zval) {
	var args *types.Zval
	...
	return_value.SetLong(types.Z_ARRVAL_P(stack).Len())
...
func ZifArrayPop(executeData zpp.DefEx, return_value zpp.DefReturn, stack zpp.DefRef) {
	...
	if types.Z_ARRVAL_P(stack).Len() == 0 {
		return
	}
...
func ZifArrayShift(executeData zpp.DefEx, return_value zpp.DefReturn, stack zpp.DefRef) {
	...
	if types.Z_ARRVAL_P(stack).IsPacked() {
		var k uint32 = 0
		if !(stack.GetArr().HasIterators()) {
			for idx = 0; idx < types.Z_ARRVAL_P(stack).GetNNumUsed(); idx++ {
				...
				if idx != k {
					var q = types.Z_ARRVAL_P(stack).Bucket(k)
					q.SetH(k)
					q.SetKey(nil)
			...
			var iter_pos = types.ZendHashIteratorsLowerPos(stack.GetArr(), 0)
			for idx = 0; idx < types.Z_ARRVAL_P(stack).GetNNumUsed(); idx++ {
				p = types.Z_ARRVAL_P(stack).Bucket(idx)
				if p.GetVal().IsUndef() {
					continue
...
func ZifArrayUnshift(executeData zpp.DefEx, return_value zpp.DefReturn, stack zpp.DefRef, _ zpp.DefOpt, vars []*types.Zval) {
	var args *types.Zval
	var stack *types.Zval
	...
	&new_hash = types.MakeArrayEx(types.Z_ARRVAL_P(stack).Len()+argc, zend.ZVAL_PTR_DTOR, 0)
	for i = 0; i < argc; i++ {
		args[i].TryAddRefcount()
		new_hash.NextIndexInsertNew(&args[i])
	}
	var __ht = stack.GetArr()
	for _, _p := range __ht.ForeachData() {
		var _z = _p.GetVal()
	...
	if stack.GetArr().HasIterators() {
		...
		new_hash.SetIteratorsCount(types.Z_ARRVAL_P(stack).GetIteratorsCount())
		stack.GetArr().SetIteratorsCount(0)
	}
	...
	types.Z_ARRVAL_P(stack).SetBy(&new_hash)
...
func ZifArraySplice(executeData zpp.DefEx, return_value zpp.DefReturn, arg zpp.DefRef, offset *types.Zval, _ zpp.DefOpt, length *types.Zval, replacement *types.Zval) {
	var array *types.Zval
	...
	num_in = types.Z_ARRVAL_P(array).Len()
...
func ZifArraySlice(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, offset *types.Zval, _ zpp.DefOpt, length *types.Zval, preserveKeys *types.Zval) {
	...
	num_in = types.Z_ARRVAL_P(input).Len()
	...
	pos = 0
	var __ht = types.Z_ARRVAL_P(input)
	for _, _p := range __ht.ForeachData() {
		var _z = _p.GetVal()
...
func PhpArrayMergeWrapper(executeData *zend.ZendExecuteData, return_value *types.Zval, recursive int) {
	...
	for i = 0; i < argc; i++ {
		...
		count += types.Z_ARRVAL_P(arg).Len()
	}
	if argc == 2 {
		var ret *types.Zval = nil
		if types.Z_ARRVAL(args[0]).Len() == 0 {
			ret = &args[1]
		} else if types.Z_ARRVAL(args[1]).Len() == 0 {
			ret = &args[0]
		}
		if ret != nil {
			if types.Z_ARRVAL_P(ret).IsPacked() {
...
func ZifArrayMerge(executeData zpp.DefEx, return_value zpp.DefReturn, _ zpp.DefOpt, arrays []*types.Zval) {
	PhpArrayMergeWrapper(executeData, return_value, 0)
}
func ZifArrayMergeRecursive(executeData zpp.DefEx, return_value zpp.DefReturn, _ zpp.DefOpt, arrays []*types.Zval) {
	PhpArrayMergeWrapper(executeData, return_value, 1)
}
func ZifArrayReplace(executeData zpp.DefEx, return_value zpp.DefReturn, arr1 *types.Zval, _ zpp.DefOpt, arrays []*types.Zval) {
	PhpArrayReplaceWrapper(executeData, return_value, 0)
}
func ZifArrayReplaceRecursive(executeData zpp.DefEx, return_value zpp.DefReturn, arr1 *types.Zval, _ zpp.DefOpt, arrays []*types.Zval) {
	PhpArrayReplaceWrapper(executeData, return_value, 1)
}
func ZifArrayKeys(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, _ zpp.DefOpt, searchValue *types.Zval, strict *types.Zval) {
	var input *types.Zval
...
func ArrayColumnFetchProp(data *types.Zval, name *types.Zval, rv *types.Zval) *types.Zval {
	...
	if data.IsType(types.IS_OBJECT) {
		...
		if name.IsType(types.IS_STRING) {
			...
			prop = data.GetArr().IndexFind(name.GetLval())
		}
		if prop != nil {
			prop = types.ZVAL_DEREF(prop)
			prop.TryAddRefcount()
...
func ZifArrayColumn(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, columnKey *types.Zval, _ zpp.DefOpt, indexKey *types.Zval) {
...
func ZifArrayReverse(executeData zpp.DefEx, return_value zpp.DefReturn, input *types.Zval, _ zpp.DefOpt, preserveKeys *types.Zval) {
	var input *types.Zval
	var entry *types.Zval
	var string_key *types.String
	var num_key zend.ZendUlong
	...
	zend.ArrayInitSize(return_value, types.Z_ARRVAL_P(input).Len())
	if types.Z_ARRVAL_P(input).IsPacked() && preserve_keys == 0 {
		types.ZendHashRealInitPacked(return_value.GetArr())
		fillScope := types.PackedFillStart(return_value.GetArr())
		var __ht = input.GetArr()
...
func ZifArrayPad(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, padSize *types.Zval, padValue *types.Zval) {
	...
	input_size = types.Z_ARRVAL_P(input).Len()
	...
	if types.Z_ARRVAL_P(input).IsPacked() {
		types.ZendHashRealInitPacked(return_value.GetArr())
		if pad_size < 0 {
			fillScope := types.PackedFillStart(return_value.GetArr())
...
func ZifArrayFlip(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval) {
	...
	zend.ArrayInitSize(return_value, types.Z_ARRVAL_P(array).Len())
	var __ht = array.GetArr()
	for _, _p := range __ht.ForeachData() {
		var _z = _p.GetVal()
...
func ZifArrayChangeKeyCase(executeData zpp.DefEx, return_value zpp.DefReturn, input *types.Zval, _ zpp.DefOpt, case_ *types.Zval) {
	var array *types.Zval
	var entry *types.Zval
	var string_key *types.String
...
func ZifArrayUnique(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, _ zpp.DefOpt, flags *types.Zval) {
	var array *types.Zval
	var idx uint32
	var p *types.Bucket
	...
	if types.Z_ARRVAL_P(array).Len() <= 1 {
	...
	if sort_type == PHP_SORT_STRING {
		...
		var val *types.Zval
		&seen = types.MakeArrayEx(types.Z_ARRVAL_P(array).Len(), nil, 0)
		zend.ArrayInit(return_value)
		var __ht = array.GetArr()
		for _, _p := range __ht.ForeachData() {
			var _z = _p.GetVal()
	...
	for ; idx < types.Z_ARRVAL_P(array).GetNNumUsed(); idx++ {
		p = types.Z_ARRVAL_P(array).Bucket(idx)
		if p.GetVal().IsUndef() {
			continue
		}
...
func PhpArrayIntersectKey(executeData *zend.ZendExecuteData, return_value *types.Zval, data_compare_type int) {
	...
	for idx = 0; idx < types.Z_ARRVAL(args[0]).GetNNumUsed(); idx++ {
		p = types.Z_ARRVAL(args[0]).Bucket(idx)
		val = p.GetVal()
		if val.IsUndef() {
			continue
		}
...
func ArrayBucketPSawp(p any, q any) {
	var t *types.Bucket
	var f = (**types.Bucket)(p)
	var g = (**types.Bucket)(q)
	t = *f
	*f = *g
	*g = t
}
func ZifArrayMultisort(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	ARRAYG__().multisort_func = (*types.CompareFuncT)(zend.Ecalloc(argc, b.SizeOf("compare_func_t")))
	...
	array_size = types.Z_ARRVAL_P(arrays[0]).Len()
	for i = 0; i < num_arrays; i++ {
		if types.Z_ARRVAL_P(arrays[i]).Len() != uint32(array_size) {
	...
	for i = 0; i < num_arrays; i++ {
		...
		for idx = 0; idx < types.Z_ARRVAL_P(arrays[i]).GetNNumUsed(); idx++ {
			p = types.Z_ARRVAL_P(arrays[i]).Bucket(idx)
			if p.GetVal().IsUndef() {
				continue
			}
			indirect[k][i] = *p
...
func ZifArrayRand(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, _ zpp.DefOpt, numReq *types.Zval) {
	var input *types.Zval
	var num_req = 1
	var string_key *types.String
	var num_key zend.ZendUlong
	...
	num_avail = types.Z_ARRVAL_P(input).Len()
	if num_avail == 0 {
...
func ZifArrayReduce(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, callback *types.Zval, _ zpp.DefOpt, initial *types.Zval) {
	var input *types.Zval
	var args []types.Zval
...
func ZifArrayFilter(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, _ zpp.DefOpt, callback *types.Zval, useKeys *types.Zval) {
	var array *types.Zval
	var operand *types.Zval
	var key *types.Zval
	var args []types.Zval
	...
	zend.ArrayInit(return_value)
	if types.Z_ARRVAL_P(array).Len() == 0 {
		zend.ZendReleaseFcallInfoCache(&fci_cache)
		return
	}
...
func ZifArrayMap(executeData zpp.DefEx, return_value zpp.DefReturn, callback *types.Zval, arrays []*types.Zval) {
	...
	if n_arrays == 1 {
		...
		maxlen = types.Z_ARRVAL(arrays[0]).Len()
		...
		for i = 0; i < n_arrays; i++ {
			...
			if types.Z_ARRVAL(arrays[i]).Len() > maxlen {
				maxlen = types.Z_ARRVAL(arrays[i]).Len()
			}
		}
		zend.ArrayInitSize(return_value, maxlen)
				...
				for i = 0; i < n_arrays; i++ {
					...
					for true {
						if pos >= types.Z_ARRVAL(arrays[i]).GetNNumUsed() {
							zv.SetNull()
							break
						} else if types.Z_ARRVAL(arrays[i]).GetArData()[pos].GetVal().IsNotUndef() {
...
func ZifArrayChunk(executeData zpp.DefEx, return_value zpp.DefReturn, arg *types.Zval, size *types.Zval, _ zpp.DefOpt, preserveKeys *types.Zval) {
	var num_in int
