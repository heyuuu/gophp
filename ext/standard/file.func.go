...
func ZifFlock(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			...
			if argparse.ZendParseArgResource(_arg, &res, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_RESOURCE
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if !argparse.ZendParseArgLong00(_arg, &operation) {
...
func ZifTempnam(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &dir, &dir_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &prefix, &prefix_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifPopen(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &command, &command_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifFgetss(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgResource(_arg, &fd, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_RESOURCE
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
			fp.StartOptional()
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if !argparse.ZendParseArgLong00(_arg, &bytes) {
				_expected_type = argparse.Z_EXPECTED_LONG
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifFscanf(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgResource(_arg, &file_handle, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_RESOURCE
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifFseek(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			...
			fp.StartOptional()
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if !argparse.ZendParseArgLong00(_arg, &whence) {
				_expected_type = argparse.Z_EXPECTED_LONG
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifUmask(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			...
			fp.StartOptional()
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if !argparse.ZendParseArgLong00(_arg, &mask) {
				_expected_type = argparse.Z_EXPECTED_LONG
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifRename(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &old_name, &old_name_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &new_name, &new_name_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func PhpIfFtruncate(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgResource(_arg, &fp, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_RESOURCE
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if !argparse.ZendParseArgLong00(_arg, &size) {
				_expected_type = argparse.Z_EXPECTED_LONG
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifCopy(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &source, &source_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &target, &target_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifFnmatch(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	for {
		...
		for {
			fp := argparse.FastParseStart(executeData, _min_num_args, _max_num_args, _flags)
			argparse.Z_PARAM_PROLOGUE(0, 0)
			if argparse.ZendParseArgPath(_arg, &pattern, &pattern_len, 0) == 0 {
				_expected_type = argparse.Z_EXPECTED_PATH
				_error_code = argparse.ZPP_ERROR_WRONG_ARG
				break
			}
