...
func CheckHttpHost(target *byte) int {
	...
	var colon *byte
	if b.Assign(&tmp, zend.ZendHashStrFind(&(zend.ExecutorGlobals.GetSymbolTable()), zend.ZEND_STRL("_SERVER"))) && tmp.GetType() == zend.IS_ARRAY && b.Assign(&host, zend.ZendHashStrFind(zend.Z_ARRVAL_P(tmp), zend.ZEND_STRL("HTTP_HOST"))) && host.GetType() == zend.IS_STRING {
