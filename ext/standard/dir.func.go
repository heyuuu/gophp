...
func ZmStartupDir(type_ int, module_number int) int {
	...
	DirClassEntryPtr = zend.RegisterClass("Directory", nil, PhpDirClassFunctions)
	...
	if executeData.NumArgs() == 0 {
		...
		if myself != nil {
			if lang.Assign(&tmp, types.Z_OBJPROP_P(myself).KeyFind("handle")) == nil {
				core.PhpErrorDocref("", faults.E_WARNING, "Unable to find my handle property")
				return_value.SetFalse()
				return
			}
...
func ZifGetcwd(executeData zpp.Ex, return_value zpp.Ret) {
	...
	if ret != nil {
		return_value.SetString(b.CastStrAuto(path))
		return
	} else {
		return_value.SetFalse()
		return
...
func PhpIfReaddir(executeData *zend.ZendExecuteData, return_value *types.Zval) {
	...
	if core.PhpStreamReaddir(dirp, &entry) != nil {
		return_value.SetString(entry.GetDName())
		return
	}
	return_value.SetFalse()
	return
...
func ZifGlob(executeData zpp.Ex, return_value zpp.Ret, pattern *types.Zval, _ zpp.Opt, flags *types.Zval) {
	...
	if (GLOB_AVAILABLE_FLAGS & flags) != flags {
		core.PhpErrorDocref("", faults.E_WARNING, "At least one of the passed flags is invalid or not supported on this platform")
		return_value.SetFalse()
		return
	}
...
func ZifScandir(executeData zpp.Ex, return_value zpp.Ret, dir *types.Zval, _ zpp.Opt, sortingOrder *types.Zval, context *types.Zval) {
	...
	if dirn_len < 1 {
		core.PhpErrorDocref("", faults.E_WARNING, "Directory name cannot be empty")
		return_value.SetFalse()
		return
	}
	if zcontext != nil {
