...
func PhpArrayDataCompare(a any, b any) int {
	...
	first = f.GetVal()
	second = s.GetVal()
	if first.GetType() == zend.IS_INDIRECT {
		first = first.GetZv()
	}
	if second.GetType() == zend.IS_INDIRECT {
		second = second.GetZv()
	}
...
func PhpCountRecursive(ht *zend.HashTable) zend.ZendLong {
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			if element.GetType() == zend.IS_ARRAY {
...
func ZifEnd(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if zend.USED_RET() {
		...
		if entry.GetType() == zend.IS_INDIRECT {
			entry = entry.GetZv()
		}
...
func PhpArrayWalk(array *zend.Zval, userdata *zend.Zval, recursive int) int {
	...
	for {
		...
		if zv.GetType() == zend.IS_INDIRECT {
			zv = zv.GetZv()
			...
			if zv.GetType() != zend.IS_REFERENCE && array.GetType() == zend.IS_OBJECT {
		...
		if recursive != 0 && zend.Z_REFVAL_P(zv).GetType() == zend.IS_ARRAY {
			...
			if zend.Z_REFVAL(ref).GetType() == zend.IS_ARRAY && thash == zend.Z_REFVAL(ref).GetArr() {
		...
		} else if array.GetType() == zend.IS_OBJECT {
...
func PhpSearchArray(execute_data *zend.ZendExecuteData, return_value *zend.Zval, behavior int) {
	...
	if strict != 0 {
		if value.GetType() == zend.IS_LONG {
			for {
				...
				for ; _p != _end; _p++ {
					...
					if entry.GetType() == zend.IS_LONG && entry.GetLval() == value.GetLval() {
						if behavior == 0 {
							zend.RETVAL_TRUE
							return
						} else {
		...
		} else if value.GetType() == zend.IS_STRING {
			for {
				var __ht *zend.HashTable = array.GetArr()
				var _p *zend.Bucket = __ht.GetArData()
				var _end *zend.Bucket = _p + __ht.GetNNumUsed()
...
func PhpExtractRefIfExists(arr *zend.ZendArray, symbol_table *zend.ZendArray) zend.ZendLong {
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			if orig_var != nil {
				if orig_var.GetType() == zend.IS_INDIRECT {
					orig_var = orig_var.GetZv()
					if orig_var.GetType() == zend.IS_UNDEF {
						continue
					}
				}
			...
			if orig_var != nil {
				...
				if PhpValidVarName(zend.Z_STRVAL(final_name), zend.Z_STRLEN(final_name)) != 0 {
					if zend.ZendStringEqualsLiteral(final_name.GetStr(), "this") {
						...
						if b.Assign(&orig_var, zend.ZendHashFind(symbol_table, final_name.GetStr())) != nil {
							if orig_var.GetType() == zend.IS_INDIRECT {
								orig_var = orig_var.GetZv()
							}
							zend.ZvalPtrDtor(orig_var)
							...
							zend.ZendHashAddNew(symbol_table, final_name.GetStr(), entry)
						}
						count++
					}
				}
...
func PhpCompactVar(eg_active_symbol_table *zend.HashTable, return_value *zend.Zval, entry *zend.Zval) {
	...
	if entry.GetType() == zend.IS_STRING {
		if b.Assign(&value_ptr, zend.ZendHashFindInd(eg_active_symbol_table, entry.GetStr())) != nil {
			...
			zend.ZendHashUpdate(return_value.GetArr(), entry.GetStr(), value_ptr)
			...
			if object != nil {
				...
				zend.ZendHashUpdate(return_value.GetArr(), entry.GetStr(), &data)
			}
		} else {
			core.PhpErrorDocref(nil, zend.E_NOTICE, "Undefined variable: %s", entry.GetStr().GetVal())
		}
	} else if entry.GetType() == zend.IS_ARRAY {
...
func ZifCompact(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if num_args != 0 && args[0].IsType(zend.IS_ARRAY) {
		zend.ArrayInitSize(return_value, args[0].GetArr().GetNNumOfElements())
	} else {
...
func ZifArrayFill(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if num > 0 {
		if b.SizeOf("num") > 4 && num > 0x7fffffff {
			...
			zend.ZendHashIndexAddNew(return_value.GetArr(), start_key, val)
			for b.PreDec(&num) {
				zend.ZendHashNextIndexInsertNew(return_value.GetArr(), val)
				start_key++
			}
		}
	} else if num == 0 {
...
func ZifArrayFillKeys(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.ArrayInitSize(return_value, keys.GetArr().GetNNumOfElements())
	for {
		...
		for ; _p != _end; _p++ {
			...
			if entry.GetType() == zend.IS_LONG {
				zend.ZendHashIndexUpdate(return_value.GetArr(), entry.GetLval(), val)
			} else {
...
func ZifRange(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if zstep != nil {
		if zstep.GetType() == zend.IS_DOUBLE {
			is_step_double = 1
		} else if zstep.GetType() == zend.IS_STRING {
	...
	if zlow.GetType() == zend.IS_STRING && zhigh.GetType() == zend.IS_STRING && zend.Z_STRLEN_P(zlow) >= 1 && zend.Z_STRLEN_P(zhigh) >= 1 {
		var type1 int
		var type2 int
		var low uint8
		var high uint8
		...
		if low > high {
			...
			for {
				...
				zend.ZEND_ASSERT((__fill_ht.GetUFlags() & zend.HASH_FLAG_PACKED) != 0)
	...
	} else if zlow.GetType() == zend.IS_DOUBLE || zhigh.GetType() == zend.IS_DOUBLE || is_step_double != 0 {
		var low float64
		var high float64
		var element float64
		var i uint32
...
func PhpArrayDataShuffle(array *zend.Zval) {
	...
	var n_left uint32
	n_elems = array.GetArr().GetNNumOfElements()
	if n_elems < 1 {
		return
	}
	hash = array.GetArr()
	...
	if (hash.GetUFlags() & zend.HASH_FLAG_PACKED) == 0 {
...
func PhpSplice(in_hash *zend.HashTable, offset zend.ZendLong, length zend.ZendLong, replace *zend.HashTable, removed *zend.HashTable) {
	...
	for ; pos < offset && idx < in_hash.GetNNumUsed(); idx++ {
		...
		if p.GetKey() == nil {
			...
			zend.ZendHashAddNew(&out_hash, p.GetKey(), entry)
		}
		if idx == iter_pos {
	...
	if removed != nil {
		for ; pos < offset+length && idx < in_hash.GetNNumUsed(); idx++ {
			...
			if p.GetKey() == nil {
				...
				zend.ZendHashAddNew(removed, p.GetKey(), entry)
	...
	in_hash.GetUFlags() = out_hash.GetUFlags()
...
func ZifArrayPush(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for i = 0; i < argc; i++ {
		...
		if zend.ZendHashNextIndexInsert(stack.GetArr(), &new_var) == nil {
	...
	zend.RETVAL_LONG(stack.GetArr().GetNNumOfElements())
...
func ZifArrayPop(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if stack.GetArr().GetNNumOfElements() == 0 {
		return
	}
	...
	if p.GetKey() != nil && stack.GetArr() == &(zend.ExecutorGlobals.GetSymbolTable()) {
		zend.ZendDeleteGlobalVariable(p.GetKey())
	} else {
...
func ZifArrayShift(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if (stack.GetArr().GetUFlags() & zend.HASH_FLAG_PACKED) != 0 {
		var k uint32 = 0
		...
		zend.Z_ARRVAL_P(stack).SetNNextFreeElement(k)
		if should_rehash != 0 {
			zend.ZendHashRehash(stack.GetArr())
		}
	}
	zend.ZendHashInternalPointerReset(stack.GetArr())
}
func ZifArrayUnshift(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.ZendHashInit(&new_hash, stack.GetArr().GetNNumOfElements()+argc, nil, zend.ZVAL_PTR_DTOR, 0)
	for i = 0; i < argc; i++ {
	...
	if zend.HT_HAS_ITERATORS(stack.GetArr()) {
		...
		zend.HT_SET_ITERATORS_COUNT(&new_hash, stack.GetArr().GetNIteratorsCount())
	...
	stack.GetArr().GetUFlags() = new_hash.GetUFlags()
...
func ZifArraySplice(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	num_in = array.GetArr().GetNNumOfElements()
...
func ZifArraySlice(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	num_in = input.GetArr().GetNNumOfElements()

	/* We want all entries from offset to the end if length is not passed or is null */

	if zend.ZEND_NUM_ARGS() < 3 || z_length.GetType() == zend.IS_NULL {
		length = num_in
	} else {
		...
		for ; _p != _end; _p++ {
			...
			if string_key != nil {
				if b.Assign(&dest_entry, zend.ZendHashFindEx(dest, string_key, 1)) != nil {
					...
					if dest_zval.GetType() == zend.IS_ARRAY {
					...
					if dest_zval.GetType() == zend.IS_NULL {
					...
					zend.ZVAL_UNDEF(&tmp)
					if src_zval.GetType() == zend.IS_OBJECT {
						zend.ZVAL_COPY(&tmp, src_zval)
						zend.ConvertToArray(&tmp)
						src_zval = &tmp
					}
					if src_zval.GetType() == zend.IS_ARRAY {
						...
						zend.ZendHashNextIndexInsert(dest_zval.GetArr(), src_zval)
					}
...
func PhpArrayMerge(dest *zend.HashTable, src *zend.HashTable) int {
	var src_entry *zend.Zval
	var string_key *zend.ZendString
	if (dest.GetUFlags()&zend.HASH_FLAG_PACKED) != 0 && (src.GetUFlags()&zend.HASH_FLAG_PACKED) != 0 {
		zend.ZendHashExtend(dest, dest.GetNNumOfElements()+src.GetNNumOfElements(), 1)
		for {
...
func PhpArrayMergeWrapper(execute_data *zend.ZendExecuteData, return_value *zend.Zval, recursive int) {
	...
	for i = 0; i < argc; i++ {
		...
		count += arg.GetArr().GetNNumOfElements()
	}
	if argc == 2 {
		var ret *zend.Zval = nil
		if args[0].GetArr().GetNNumOfElements() == 0 {
			ret = &args[1]
		} else if args[1].GetArr().GetNNumOfElements() == 0 {
			ret = &args[0]
		}
		if ret != nil {
			if (ret.GetArr().GetUFlags() & zend.HASH_FLAG_PACKED) != 0 {
	...
	if (src.GetUFlags() & zend.HASH_FLAG_PACKED) != 0 {
			...
			for {
				...
				for ; _p != _end; _p++ {
					...
					if zend.FastIsIdenticalFunction(search_value, entry) != 0 {
						...
						zend.ZendHashNextIndexInsertNew(return_value.GetArr(), &new_val)
					}
				}
				break
			}
		...
		for ; _p != _end; _p++ {
			...
			if entry.GetType() == zend.IS_LONG {
				if b.Assign(&tmp, zend.ZendHashIndexFind(return_value.GetArr(), entry.GetLval())) == nil {
					...
					zend.ZendHashIndexUpdate(return_value.GetArr(), entry.GetLval(), &data)
				} else {
					tmp.GetLval()++
				}
			} else if entry.GetType() == zend.IS_STRING {
...
func ArrayColumnFetchProp(data *zend.Zval, name *zend.Zval, rv *zend.Zval) *zend.Zval {
	...
	if data.GetType() == zend.IS_OBJECT {
	...
	} else if data.GetType() == zend.IS_ARRAY {
		if name.GetType() == zend.IS_STRING {
			prop = zend.ZendSymtableFind(data.GetArr(), name.GetStr())
		} else if name.GetType() == zend.IS_LONG {
				...
				if index != nil {
					...
					if keyval != nil {
						...
						case zend.IS_STRING:
							...
							break
						case zend.IS_LONG:
							zend.ZendHashIndexUpdate(return_value.GetArr(), keyval.GetLval(), colval)
							break
						case zend.IS_OBJECT:
							var tmp_key *zend.ZendString
							...
							break
						case zend.IS_NULL:
							zend.ZendHashUpdate(return_value.GetArr(), zend.ZSTR_EMPTY_ALLOC(), colval)
							break
						case zend.IS_DOUBLE:
							zend.ZendHashIndexUpdate(return_value.GetArr(), zend.ZendDvalToLval(keyval.GetDval()), colval)
							break
						case zend.IS_TRUE:
							zend.ZendHashIndexUpdate(return_value.GetArr(), 1, colval)
							break
						case zend.IS_FALSE:
							zend.ZendHashIndexUpdate(return_value.GetArr(), 0, colval)
							break
						case zend.IS_RESOURCE:
							zend.ZendHashIndexUpdate(return_value.GetArr(), zend.Z_RES_HANDLE_P(keyval), colval)
							break
						default:
							zend.ZendHashNextIndexInsert(return_value.GetArr(), colval)
							break
						}
...
func ZifArrayReverse(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.ArrayInitSize(return_value, input.GetArr().GetNNumOfElements())
	if (input.GetArr().GetUFlags()&zend.HASH_FLAG_PACKED) != 0 && preserve_keys == 0 {
		zend.ZendHashRealInitPacked(return_value.GetArr())
		for {
			var __fill_ht *zend.HashTable = return_value.GetArr()
			var __fill_bkt *zend.Bucket = __fill_ht.GetArData() + __fill_ht.GetNNumUsed()
...
func ZifArrayPad(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	input_size = input.GetArr().GetNNumOfElements()
	...
	if (input.GetArr().GetUFlags() & zend.HASH_FLAG_PACKED) != 0 {
		zend.ZendHashRealInitPacked(return_value.GetArr())
		if pad_size < 0 {
			var __fill_ht *zend.HashTable = return_value.GetArr()
			var __fill_bkt *zend.Bucket = __fill_ht.GetArData() + __fill_ht.GetNNumUsed()
		...
		if pad_size < 0 {
			for i = 0; i < num_pads; i++ {
				zend.ZendHashNextIndexInsertNew(return_value.GetArr(), pad_value)
			}
		}
		for {
			var __ht *zend.HashTable = input.GetArr()
			...
			for ; _p != _end; _p++ {
				...
				zend.Z_TRY_ADDREF_P(value)
				if key != nil {
					zend.ZendHashAddNew(return_value.GetArr(), key, value)
				} else {
					zend.ZendHashNextIndexInsertNew(return_value.GetArr(), value)
				}
			}
			break
		}
...
func ZifArrayFlip(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.ArrayInitSize(return_value, array.GetArr().GetNNumOfElements())
	for {
		var __ht *zend.HashTable = array.GetArr()
		var _p *zend.Bucket = __ht.GetArData()
		var _end *zend.Bucket = _p + __ht.GetNNumUsed()
...
func ZifArrayChangeKeyCase(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			entry = _z
			if string_key == nil {
				entry = zend.ZendHashIndexUpdate(return_value.GetArr(), num_key, entry)
			} else {
				...
				entry = zend.ZendHashUpdate(return_value.GetArr(), new_key, entry)
...
func ZifArrayUnique(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if sort_type == PHP_SORT_STRING {
		...
		var val *zend.Zval
		zend.ZendHashInit(&seen, array.GetArr().GetNNumOfElements(), nil, nil, 0)
		zend.ArrayInit(return_value)
		for {
			var __ht *zend.HashTable = array.GetArr()
			var _p *zend.Bucket = __ht.GetArData()
			...
			for ; _p != _end; _p++ {
				...
				if retval != nil {
					...
					zend.Z_TRY_ADDREF_P(val)
					if str_key != nil {
						zend.ZendHashAddNew(return_value.GetArr(), str_key, val)
					} else {
						zend.ZendHashIndexAddNew(return_value.GetArr(), num_key, val)
					}
				}
			}
			break
	...
	arTmp = (*Bucketindex)(zend.Pemalloc((zend.Z_ARRVAL_P(array).GetNNumOfElements()+1)*b.SizeOf("struct bucketindex"), zend.GC_FLAGS(array.GetArr())&zend.IS_ARRAY_PERSISTENT))
	i = 0
	idx = 0
	for ; idx < zend.Z_ARRVAL_P(array).GetNNumUsed(); idx++ {
		...
		if p.GetVal().IsType(zend.IS_INDIRECT) && p.GetVal().GetZv().GetType() == zend.IS_UNDEF {
			continue
		}
		arTmp[i].SetB(*p)
		arTmp[i].SetI(i)
	...
	for cmpdata = arTmp + 1; cmpdata.GetB().GetVal().GetType() != zend.IS_UNDEF; cmpdata++ {
		if cmp(lastkept, cmpdata) != 0 {
			...
			if p.GetKey() == nil {
				...
				if return_value.GetArr() == &(zend.ExecutorGlobals.GetSymbolTable()) {
					zend.ZendDeleteGlobalVariable(p.GetKey())
				} else {
	...
	zend.Pefree(arTmp, zend.GC_FLAGS(array.GetArr())&zend.IS_ARRAY_PERSISTENT)
}
...
func PhpArrayIntersectKey(execute_data *zend.ZendExecuteData, return_value *zend.Zval, data_compare_type int) {
	...
	for idx = 0; idx < zend.Z_ARRVAL(args[0]).GetNNumUsed(); idx++ {
		...
		if p.GetKey() == nil {
			...
			if ok != 0 {
				zend.Z_TRY_ADDREF_P(val)
				zend.ZendHashIndexUpdate(return_value.GetArr(), p.GetH(), val)
			}
		} else {
			ok = 1
			for i = 1; i < argc; i++ {
			...
			if ok != 0 {
				zend.Z_TRY_ADDREF_P(val)
				zend.ZendHashUpdate(return_value.GetArr(), p.GetKey(), val)
			}
		}
	}
}
...
func ZifArrayDiff(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	num = args[0].GetArr().GetNNumOfElements()
	if num == 0 {
		for i = 1; i < argc; i++ {
			if args[i].GetType() != zend.IS_ARRAY {
				core.PhpErrorDocref(nil, zend.E_WARNING, "Expected parameter %d to be an array, %s given", i+1, zend.ZendZvalTypeName(&args[i]))
	...
	for i = 1; i < argc; i++ {
		...
		num += args[i].GetArr().GetNNumOfElements()
	}
	if num == 0 {
		zend.ZVAL_COPY(return_value, &args[0])
		return
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			if zend.ZendHashExists(&exclude, str) == 0 {
				if key != nil {
					value = zend.ZendHashAddNew(return_value.GetArr(), key, value)
				} else {
					value = zend.ZendHashIndexAddNew(return_value.GetArr(), idx, value)
				}
...
func ZifArrayMultisort(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for i = 0; i < argc; i++ {
		...
		} else if arg.GetType() == zend.IS_LONG {
	...
	array_size = arrays[0].GetArr().GetNNumOfElements()
	for i = 0; i < num_arrays; i++ {
		if arrays[i].GetArr().GetNNumOfElements() != uint32(array_size) {
	...
	for i = 0; i < num_arrays; i++ {
		...
		if repack != 0 {
			zend.ZendHashToPacked(hash)
		} else if (hash.GetUFlags() & zend.HASH_FLAG_PACKED) == 0 {
...
func ZifArrayRand(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	num_avail = input.GetArr().GetNNumOfElements()
	if num_avail == 0 {
...
func ZifArraySum(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			entry = _z
			if entry.GetType() == zend.IS_ARRAY || entry.GetType() == zend.IS_OBJECT {
				continue
			}
...
func ZifArrayProduct(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if !(input.GetArr().GetNNumOfElements()) {
		return
	}
	for {
		var __ht *zend.HashTable = input.GetArr()
		...
		for ; _p != _end; _p++ {
			...
			zend.ZVAL_COPY(&entry_n, entry)
			zend.ConvertScalarToNumber(&entry_n)
			if entry_n.IsType(zend.IS_LONG) && return_value.GetType() == zend.IS_LONG {
				...
				if float64(zend.ZEND_LONG_MIN <= dval && dval <= float64(zend.ZEND_LONG_MAX)) {
					return_value.GetLval() *= entry_n.GetLval()
					continue
				}
			}
			zend.ConvertToDouble(return_value)
			zend.ConvertToDouble(&entry_n)
			return_value.GetDval() *= entry_n.GetDval()
		}
		break
	}
}
...
func ZifArrayFilter(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.ArrayInit(return_value)
	if array.GetArr().GetNNumOfElements() == 0 {
		zend.ZendReleaseFcallInfoCache(&fci_cache)
		return
	}
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			if string_key != nil {
				operand = zend.ZendHashUpdate(return_value.GetArr(), string_key, operand)
			} else {
				operand = zend.ZendHashIndexUpdate(return_value.GetArr(), num_key, operand)
			}
...
func ZifArrayMap(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if n_arrays == 1 {
		...
		maxlen = arrays[0].GetArr().GetNNumOfElements()
		...
		zend.ZendHashRealInit(return_value.GetArr(), arrays[0].GetArr().GetUFlags()&zend.HASH_FLAG_PACKED)
		for {
		...
		for i = 0; i < n_arrays; i++ {
			...
			if arrays[i].GetArr().GetNNumOfElements() > maxlen {
				maxlen = arrays[i].GetArr().GetNNumOfElements()
			}
		}
		zend.ArrayInitSize(return_value, maxlen)
		if !(zend.ZEND_FCI_INITIALIZED(fci)) {
			...
			for k = 0; k < maxlen; k++ {
				...
				for i = 0; i < n_arrays; i++ {
					...
					zend.ZendHashNextIndexInsertNew(result.GetArr(), &zv)
				}
				zend.ZendHashNextIndexInsertNew(return_value.GetArr(), &result)
			}
...
func ZifArrayChunk(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			if preserve_keys != 0 {
				if str_key != nil {
					entry = zend.ZendHashUpdate(chunk.GetArr(), str_key, entry)
				} else {
					entry = zend.ZendHashIndexUpdate(chunk.GetArr(), num_key, entry)
				}
			} else {
				entry = zend.ZendHashNextIndexInsert(chunk.GetArr(), entry)
			}
			zend.ZvalAddRef(entry)
		...
		for ; _p != _end; _p++ {
			...
			for true {
				if pos_values >= values.GetNNumUsed() {
					...
					if entry_keys.GetType() == zend.IS_LONG {
						entry_values = zend.ZendHashIndexUpdate(return_value.GetArr(), entry_keys.GetLval(), entry_values)
					} else {
						var tmp_key *zend.ZendString
