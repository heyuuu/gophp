...
func PhpExtractRefOverwrite(arr *zend.ZendArray, symbol_table *zend.ZendArray) zend.ZendLong {
	...
	for _, _p := range __ht.foreachData() {
		...
		if orig_var != nil {
			...
			zend.ZendHashAddNew(symbol_table, var_name.GetStr(), entry)
		}
		count++
	}
	return count
			...
			if PhpValidVarName(zend.Z_STRVAL(final_name), zend.Z_STRLEN(final_name)) != 0 {
				if zend.ZendStringEqualsLiteral(final_name.GetStr(), "this") {
					...
					if b.Assign(&orig_var, symbol_table.FindByZendString(final_name.GetStr())) != nil {
						...
						zend.ZendHashAddNew(symbol_table, final_name.GetStr().GetStr(), entry)
					}
					count++
				}
			}
...
func PhpCompactVar(eg_active_symbol_table *zend.HashTable, return_value *zend.Zval, entry *zend.Zval) {
	...
	if entry.IsType(zend.IS_STRING) {
		if b.Assign(&value_ptr, zend.ZendHashFindInd(eg_active_symbol_table, entry.GetStr())) != nil {
			...
			zend.ZendHashUpdate(return_value.GetArr(), entry.GetStr().GetStr(), value_ptr)
			...
			if object != nil {
				...
				zend.ZendHashUpdate(return_value.GetArr(), entry.GetStr().GetStr(), &data)
			}
		} else {
...
func PhpSplice(in_hash *zend.HashTable, offset zend.ZendLong, length zend.ZendLong, replace *zend.HashTable, removed *zend.HashTable) {
	...
	for ; pos < offset && idx < in_hash.GetNNumUsed(); idx++ {
		...
		if p.GetKey() == nil {
			...
			zend.ZendHashAddNew(&out_hash, p.GetKey().GetStr(), entry)
		}
		if idx == iter_pos {
	...
	if removed != nil {
		for ; pos < offset+length && idx < in_hash.GetNNumUsed(); idx++ {
			...
			if p.GetKey() == nil {
				...
				zend.ZendHashAddNew(removed, p.GetKey().GetStr(), entry)
...
func ZifArrayUnshift(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for _, _p := range __ht.foreachData() {
		...
		value = _z
		if key != nil {
			zend.ZendHashAddNew(&new_hash, key.GetStr(), value)
		} else {
...
func ZifArraySlice(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for _, _p := range __ht.foreachData() {
		...
		if string_key != nil {
			entry = zend.ZendHashAddNew(return_value.GetArr(), string_key.GetStr(), entry)
		} else {
			if preserve_keys != 0 {
	...
	for _, _p := range __ht.foreachData() {
		...
		if string_key != nil {
			if b.Assign(&dest_entry, dest.FindByZendString(string_key)) != nil {
				...
				var zv *zend.Zval = zend.ZendHashAddNew(dest, string_key.GetStr(), src_entry)
				zend.ZvalAddRef(zv)
			}
		} else {
	...
	for _, _p := range __ht.foreachData() {
		...
		if string_key != nil {
			if src_zval.GetType() != zend.IS_ARRAY || b.Assign(&dest_entry, dest.FindByZendString(string_key)) == nil || dest_entry.GetType() != zend.IS_ARRAY && (!(zend.Z_ISREF_P(dest_entry)) || zend.Z_REFVAL_P(dest_entry).GetType() != zend.IS_ARRAY) {
				var zv *zend.Zval = zend.ZendHashUpdate(dest, string_key.GetStr(), src_entry)
				zend.ZvalAddRef(zv)
				continue
			}
		} else {
			...
			if index != nil {
				...
				if keyval != nil {
					...
					case zend.IS_OBJECT:
						...
						break
					case zend.IS_NULL:
						zend.ZendHashUpdate(return_value.GetArr(), zend.ZSTR_EMPTY_ALLOC().GetStr(), colval)
						break
					case zend.IS_DOUBLE:
...
func ZifArrayPad(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if zend.Z_ARRVAL_P(input).HasUFlags(zend.HASH_FLAG_PACKED) {
		...
		for _, _p := range __ht.foreachData() {
			...
			zend.Z_TRY_ADDREF_P(value)
			if key != nil {
				zend.ZendHashAddNew(return_value.GetArr(), key.GetStr(), value)
			} else {
...
func ZifArrayChangeKeyCase(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for _, _p := range __ht.foreachData() {
		...
		if string_key == nil {
			...
			entry = zend.ZendHashUpdate(return_value.GetArr(), new_key.GetStr(), entry)
	...
	if sort_type == PHP_SORT_STRING {
		...
		for _, _p := range __ht.foreachData() {
			...
			if retval != nil {
				...
				zend.Z_TRY_ADDREF_P(val)
				if str_key != nil {
					zend.ZendHashAddNew(return_value.GetArr(), str_key.GetStr(), val)
				} else {
...
func PhpArrayIntersectKey(execute_data *zend.ZendExecuteData, return_value *zend.Zval, data_compare_type int) {
	...
	for idx = 0; idx < zend.Z_ARRVAL(args[0]).GetNNumUsed(); idx++ {
		...
		if p.GetKey() == nil {
			...
			if ok != 0 {
				zend.Z_TRY_ADDREF_P(val)
				zend.ZendHashUpdate(return_value.GetArr(), p.GetKey().GetStr(), val)
			}
		}
	}
}
...
func ZifArrayDiff(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for i = 1; i < argc; i++ {
		...
		for _, _p := range __ht.foreachData() {
			...
			value = _z
			str = zend.ZvalGetTmpString(value, &tmp_str)
			zend.ZendHashAdd(&exclude, str.GetStr(), &dummy)
			zend.ZendTmpStringRelease(tmp_str)
		}
	}
	...
	for _, _p := range __ht.foreachData() {
		...
		if zend.ZendHashExists(&exclude, str) == 0 {
			if key != nil {
				value = zend.ZendHashAddNew(return_value.GetArr(), key.GetStr(), value)
			} else {
...
func ZifArrayFilter(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for _, _p := range __ht.foreachData() {
		...
		if string_key != nil {
			operand = zend.ZendHashUpdate(return_value.GetArr(), string_key.GetStr(), operand)
		} else {
...
func ZifArrayChunk(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for _, _p := range __ht.foreachData() {
		...
		if preserve_keys != 0 {
			if str_key != nil {
				entry = zend.ZendHashUpdate(chunk.GetArr(), str_key.GetStr(), entry)
			} else {
