...
func ValidateFormat(format *byte, numVars int, totalSubs *int) int {
	...
	notXpg:
		...
		if gotXpg != 0 {
		mixedXPG:
			core.PhpErrorDocref(nil, faults.E_WARNING, "%s", "cannot mix \"%\" and \"%n$\" conversion specifiers")
			goto error
		}
	xpgCheckDone:
		...
		default:
			core.PhpErrorDocref(nil, faults.E_WARNING, "Bad scan conversion character \"%c\"", *ch)
			goto error
		}
	...
	for i = 0; i < numVars; i++ {
		if nassign[i] > 1 {
			core.PhpErrorDocref(nil, faults.E_WARNING, "%s", "Variable is assigned by multiple \"%n$\" conversion specifiers")
			goto error
			...
			core.PhpErrorDocref(nil, faults.E_WARNING, "Variable is not assigned by any conversion specifiers")
			goto error
		}
	}
	if nassign != staticAssign {
...
badIndex:
	if gotXpg != 0 {
		core.PhpErrorDocref(nil, faults.E_WARNING, "%s", "\"%n$\" argument index out of range")
	} else {
		core.PhpErrorDocref(nil, faults.E_WARNING, "Different numbers of variable names and field specifiers")
	}
error:
	if nassign != staticAssign {
		zend.Efree((*byte)(nassign))
	...
	if numVars != 0 {
		for i = varStart; i < argCount; i++ {
			if !(args[i].IsReference()) {
				core.PhpErrorDocref(nil, faults.E_WARNING, "Parameter %d must be passed by reference", i)
				ScanSetErrorReturn(numVars, return_value)
