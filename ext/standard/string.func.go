...
func PhpSpnCommonHandler(execute_data *zend.ZendExecuteData, return_value *zend.Zval, behavior int) {
	...
	if behavior == STR_STRSPN {
		zend.RETVAL_LONG(PhpStrspn(s11.GetVal()+start, s22.GetVal(), s11.GetVal()+start+len_, s22.GetVal()+s22.GetLen()))
		return
	} else if behavior == STR_STRCSPN {
		zend.RETVAL_LONG(PhpStrcspn(s11.GetVal()+start, s22.GetVal(), s11.GetVal()+start+len_, s22.GetVal()+s22.GetLen()))
		return
	}
}
...
func ZifStrcoll(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.RETVAL_LONG(strcoll((*byte)(s1.GetVal()), (*byte)(s2.GetVal())))
	return
}
...
func PhpImplode(glue *zend.ZendString, pieces *zend.Zval, return_value *zend.Zval) {
	...
	numelems = pieces.GetArr().GetNNumOfElements()
	if numelems == 0 {
		zend.RETVAL_EMPTY_STRING()
		return
	} else if numelems == 1 {
	...
	for {
		...
		for ; _p != _end; _p++ {
			...
			tmp = _z
			if tmp.GetType() == zend.IS_STRING {
				ptr.str = tmp.GetStr()
				len_ += ptr.str.GetLen()
				ptr.lval = 0
				ptr++
			} else if tmp.GetType() == zend.IS_LONG {
...
func ZifImplode(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if arg2 == nil {
		...
		} else if arg2.GetType() == zend.IS_ARRAY {
...
func ZifDirname(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if levels == 1 {
		...
		ret.GetLen() = zend.ZendDirname(ret.GetVal(), str_len)
		...
		for {
			ret.GetLen() = zend.ZendDirname(ret.GetVal(), b.Assign(&str_len, ret.GetLen()))
...
func ZifStristr(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if needle.GetType() == zend.IS_STRING {
	...
	if found != nil {
		...
		if part != 0 {
			zend.RETVAL_STRINGL(haystack.GetVal(), found_offset)
		} else {
			zend.RETVAL_STRINGL(haystack.GetVal()+found_offset, haystack.GetLen()-found_offset)
		}
	} else {
		zend.RETVAL_FALSE
	}
...
func ZifStrstr(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if found != nil {
		...
		if part != 0 {
			...
			zend.RETVAL_STRINGL(found, haystack.GetLen()-found_offset)
			return
		}
	}
	zend.RETVAL_FALSE
...
func ZifStrpos(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if found != nil {
		zend.RETVAL_LONG(found - haystack.GetVal())
		return
	} else {
		zend.RETVAL_FALSE
		return
...
func ZifStrrpos(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if zneedle.GetType() == zend.IS_STRING {
...
func ZifSubstr(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if l == 0 {
		...
		zend.RETVAL_INTERNED_STR(zend.ZSTR_CHAR(zend_uchar(str.GetVal()[f])))
		return
	} else if l == str.GetLen() {
		zend.RETVAL_STR_COPY(str)
		return
	}
	zend.RETVAL_STRINGL(str.GetVal()+f, l)
	return
}
func ZifSubstrReplace(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if str.GetType() == zend.IS_STRING {
		if argc == 3 && from.GetType() == zend.IS_ARRAY || argc == 4 && from.GetType() != len_.GetType() {
			core.PhpErrorDocref(nil, zend.E_WARNING, "'start' and 'length' should be of same type - numerical or array ")
			zend.RETVAL_STR_COPY(str.GetStr())
			return
		}
		if argc == 4 && from.GetType() == zend.IS_ARRAY {
			if from.GetArr().GetNNumOfElements() != len_.GetArr().GetNNumOfElements() {
	...
	if str.GetType() != zend.IS_ARRAY {
		...
		for {
			...
			for ; _p != _end; _p++ {
				...
				if argc > 3 && len_.GetType() == zend.IS_ARRAY {
...
func ZifQuotemeta(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	*q = '0'
	zend.RETVAL_NEW_STR(zend.ZendStringTruncate(str, q-str.GetVal(), 0))
	return
}
func ZifOrd(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.RETVAL_LONG(uint8(str.GetVal()[0]))
	return
}
			...
			for {
				...
				for ; _p != _end; _p++ {
					...
					if str_key.GetLen() < 1 {
						...
						zend.RETVAL_STR(PhpCharToStrEx(str, str_key.GetVal()[0], replace.GetVal(), replace.GetLen(), 1, nil))
					} else {
						var dummy zend.ZendLong
						zend.RETVAL_STR(PhpStrToStrEx(str, str_key.GetVal(), str_key.GetLen(), replace.GetVal(), replace.GetLen(), &dummy))
					}
...
func ZifAddcslashes(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.RETVAL_STR(PhpAddcslashesStr(str.GetVal(), str.GetLen(), what.GetVal(), what.GetLen()))
	return
}
...
do_escape:
	...
	*target = '0'
	if new_str.GetLen()-(target-new_str.GetVal()) > 16 {
		...
		new_str.GetLen() = target - new_str.GetVal()
	}
	return new_str
}
...
func PhpStrReplaceInSubject(search *zend.Zval, replace *zend.Zval, subject *zend.Zval, result *zend.Zval, case_sensitivity int) zend.ZendLong {
	...
	if search.GetType() == zend.IS_ARRAY {
		...
		if replace.GetType() == zend.IS_ARRAY {
			replace_idx = 0
		} else {
		...
		zend.ZEND_ASSERT(search.GetType() == zend.IS_STRING)
...
func PhpStrReplaceCommon(execute_data *zend.ZendExecuteData, return_value *zend.Zval, case_sensitivity int) {
	...
	if subject.GetType() == zend.IS_ARRAY {
		zend.ArrayInit(return_value)
...
func ZifStripTags(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if allow != nil {
		if allow.GetType() == zend.IS_ARRAY {
			var tmp *zend.Zval
	...
	buf.GetLen() = PhpStripTagsEx(buf.GetVal(), str.GetLen(), nil, allowed_tags, allowed_tags_len, 0)
...
func PhpStrnatcmp(execute_data *zend.ZendExecuteData, return_value *zend.Zval, fold_case int) {
	...
	zend.RETVAL_LONG(StrnatcmpEx(s1.GetVal(), s1.GetLen(), s2.GetVal(), s2.GetLen(), fold_case))
	return
}
...
func ZifStrShuffle(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	zend.RETVAL_STRINGL(arg.GetVal(), arg.GetLen())
...
func ZifMoneyFormat(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	str.GetLen() = int(res_len)
	str.GetVal()[str.GetLen()] = '0'
	zend.RETVAL_NEW_STR(zend.ZendStringTruncate(str, str.GetLen(), 0))
	return
}
	...
	for haystack_ptr = haystack.GetVal(); haystack_ptr < haystack.GetVal()+haystack.GetLen(); haystack_ptr++ {
		for cl_ptr = char_list.GetVal(); cl_ptr < char_list.GetVal()+char_list.GetLen(); cl_ptr++ {
			if (*cl_ptr) == (*haystack_ptr) {
				zend.RETVAL_STRINGL(haystack_ptr, haystack.GetVal()+haystack.GetLen()-haystack_ptr)
				return
			}
		}
	}
...
func ZifSubstrCompare(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	if cs == 0 {
		zend.RETVAL_LONG(zend.ZendBinaryStrncmp(s1.GetVal()+offset, s1.GetLen()-offset, s2.GetVal(), s2.GetLen(), cmp_len))
		return
	} else {
		zend.RETVAL_LONG(zend.ZendBinaryStrncasecmpL(s1.GetVal()+offset, s1.GetLen()-offset, s2.GetVal(), s2.GetLen(), cmp_len))
		return
	}
}
