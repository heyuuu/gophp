...
func ZifGethostbyname(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for {
		...
		for {
			...
			_real_arg = zend.ZEND_CALL_ARG(execute_data, 0)
			zend.Z_PARAM_PROLOGUE(0, 0)
			if zend.UNEXPECTED(zend.ZendParseArgString(_arg, &hostname, &hostname_len, 0) == 0) {
				_expected_type = zend.Z_EXPECTED_STRING
				_error_code = zend.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifDnsCheckRecord(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for {
		...
		for {
			...
			_optional = 1
			zend.Z_PARAM_PROLOGUE(0, 0)
			if zend.UNEXPECTED(zend.ZendParseArgString(_arg, &rectype, &rectype_len, 0) == 0) {
				_expected_type = zend.Z_EXPECTED_STRING
				_error_code = zend.ZPP_ERROR_WRONG_ARG
				break
			}
...
func ZifDnsGetRecord(execute_data *zend.ZendExecuteData, return_value *zend.Zval) {
	...
	for {
		...
		for {
			...
			_optional = 1
			zend.Z_PARAM_PROLOGUE(0, 0)
			if zend.UNEXPECTED(zend.ZendParseArgLong(_arg, &type_param, &_dummy, 0, 0) == 0) {
			...
			zend.Z_PARAM_PROLOGUE(0, 0)
			if zend.UNEXPECTED(zend.ZendParseArgBool(_arg, &raw, &_dummy, 0) == 0) {
	...
	for ; type_ < b.Cond(addtl != nil, PHP_DNS_NUM_TYPES+2, PHP_DNS_NUM_TYPES) || first_query != 0; type_++ {
		...
		if type_to_fetch != 0 {
			...
			for b.PostDec(&an) && cp != nil && cp < end {
				...
				if zend.Z_TYPE(retval) != zend.IS_UNDEF && store_results != 0 {
