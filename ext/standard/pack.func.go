...
func ZifPack(executeData zpp.Ex, return_value zpp.Ret, format *types.Zval, _ zpp.Opt, args []*types.Zval) {
	...
	if currentarg < num_args {
		core.PhpErrorDocref(nil, faults.E_WARNING, "%d arguments unused", num_args-currentarg)
	}
	...
	for i = 0; i < formatcount; i++ {
		...
		case 'X':
			outputpos -= arg
			if outputpos < 0 {
				core.PhpErrorDocref(nil, faults.E_WARNING, "Type %c: outside of string", code)
				outputpos = 0
			}
		case '@':
			outputpos = arg
...
func ZifUnpack(executeData zpp.Ex, return_value zpp.Ret, format *types.Zval, input *types.Zval, _ zpp.Opt, offset *types.Zval) {
	...
	if offset < 0 || offset > inputlen {
		core.PhpErrorDocref(nil, faults.E_WARNING, "Offset "+zend.ZEND_LONG_FMT+" is out of input range", offset)
		return_value.SetFalse()
		return
	}
	input += offset
	...
	for b.PostDec(&formatlen) > 0 {
		...
		case 'X':
			size = -1
			if arg < 0 {
				core.PhpErrorDocref(nil, faults.E_WARNING, "Type %c: '*' ignored", type_)
				arg = 1
			}
		case '@':
			size = 0
			...
			if inputpos+size <= inputlen {
				...
				case 'X':
					if inputpos < size {
						...
						i = arg - 1
						if arg >= 0 {
							core.PhpErrorDocref(nil, faults.E_WARNING, "Type %c: outside of string", type_)
						}
					}
				case '@':
					if arg <= inputlen {
